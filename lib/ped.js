...
export async function handlePedCarouselDownload(request, response) {
  ...
  for (const row of rows) {
    ...
    if (!await isInsideDriveRoot(row.drive_file_id, rootId, metadata)) {
      sendJson(response, 403, { error: "Un file del carosello non appartiene piu al Drive del cliente" });
      return;
    }
    const download = await driveDownloadResponse(metadata);
    const baseName = safeFilename(download.filename || metadata.name, `contenuto-${downloads.length + 1}`);
    ...
    const filename = seen
      ...
      : baseName;
    downloads.push({ response: download.response, filename });
  }
