...
export function carouselArchiveFilename(value, index) {
  const position = Number(index) + 1;
  const safePosition = Number.isInteger(position) && position > 0 ? position : 1;
  const prefix = String(safePosition).padStart(2, "0");
  return `${prefix} - ${safeFilename(value, `contenuto-${safePosition}`)}`;
}
...
export async function handlePedCarouselDownload(request, response) {
  ...
  try {
    prepared = await mapWithConcurrency(rows, CAROUSEL_DOWNLOAD_CONCURRENCY, async (row, index) => {
      ...
      // The authenticated PED row is the authorization boundary for imported
      // media that can live outside the client root.
  ...
  const downloads = prepared.map((download, index) => ({
    ...download,
    filename: carouselArchiveFilename(download.baseName, index)
  }));
...
export default async function handler(request, response) {
  ...
  if (request.method === "GET") {
    ...
    const rootId = driveFolderId(client.drive_url);
    const [result, agendaResult, usedResult] = await Promise.all([
    ...
    const [rows, agendaRows, usedRows] = await Promise.all([result.json(), agendaResult.json(), usedResult.json()]);
    sendJson(response, 200, {
      ...
      used_file_ids: [...new Set((usedRows || []).map((row) => String(row.drive_file_id || "")).filter(Boolean))]
    });
    return;
  }
  ...
  if (request.method === "PATCH") {
    ...
    if (Array.isArray(body.append_drive_file_ids)) {
      ...
      const payload = metadataList.map((metadata, index) => ({
        ...
        instagram_position: first.instagram_position,
      ...
      if (!result.ok) {
        const duplicate = rows?.code === "23505";
        sendJson(response, duplicate ? 409 : result.status, {
          error: duplicate ? "Uno dei contenuti selezionati e gia presente nel PED in questa data" : rows?.message || "Impossibile aggiornare il carosello"
        });
        return;
      }
