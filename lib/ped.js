...
function requestedDriveSource(body) {
  const source = String(body?.drive_source || "").trim().toLowerCase();
  return source === "all" ? "all" : "";
}

async function allowedPedDriveRootIds(client, clientRootId, driveSource = "") {
  const libraryRoots = await resolveClientDriveLibraries(client.name, listDriveFolder, clientConnectionSettings(client.notes)).catch(() => []);
  const rootIds = [clientRootId, ...libraryRoots.map((library) => library.id)];
  let fullDriveRootId = "";
  if (driveSource === "all") {
    const driveRoot = await driveMetadataWithWriteAccess("root");
    fullDriveRootId = String(driveRoot?.id || "");
  }
  return { rootIds: [...new Set(rootIds.filter(Boolean))], fullDriveRootId };
}

async function allowedPedDriveFile(fileId, authorization) {
  let metadata = null;
  try {
    metadata = await driveMetadata(fileId, { fresh: true });
    for (const rootId of authorization.rootIds) {
      if (await isInsideDriveRoot(fileId, rootId, metadata)) return { allowed: true, metadata };
    }
  } catch {
    // I file visibili soltanto all'account Drive operativo vengono verificati
    // subito sotto attraverso la radice OAuth completa.
  }
  if (!authorization.fullDriveRootId) return { allowed: false, metadata };
  const oauthMetadata = await driveMetadataWithWriteAccess(fileId);
  const allowed = await isInsideDriveRootWithWriteAccess(fileId, authorization.fullDriveRootId, oauthMetadata);
  return { allowed, metadata: oauthMetadata };
}
...
async function clientById(clientId) {
  const result = await supabaseFetch(`/clients?select=id,name,drive_url,notes&id=eq.${encodeURIComponent(clientId)}&limit=1`);
...
function requestedFileIds(body) {
  const raw = Array.isArray(body.drive_file_ids) ? body.drive_file_ids : [body.drive_file_id];
  return [...new Set(raw.map((value) => String(value || "").trim()).filter(Boolean))];
}

export function automaticPedContentType(requestedType, metadataList) {
  if (contentType(requestedType) === "story") return "story";
  if ((metadataList || []).length > 1) return "carousel";
  return String(metadataList?.[0]?.mimeType || "").toLowerCase().startsWith("video/") ? "reel" : "post";
}
...
export default async function handler(request, response) {
  ...
  if (request.method === "GET") {
    ...
    const [result, agendaResult, usedResult, notesResult, stagingResult] = await Promise.all([
      ...
      supabaseFetch(`/ped_items?select=*&client_id=eq.${encodeURIComponent(clientId)}${includeHistory ? "" : `&scheduled_date=gte.${agendaFrom}`}&order=scheduled_date.asc,position.asc,content_group_id.asc,group_position.asc,created_at.asc`),
  ...
  if (request.method === "POST") {
    ...
    const requestedFormat = contentType(body.content_type);
    let fileIds = requestedFileIds(body);
    if (requestedFormat === "story") fileIds = fileIds.slice(0, 1);
    ...
    if (fileIds.length > MAX_CAROUSEL_FILES) {
      sendJson(response, 400, { error: `Puoi selezionare al massimo ${MAX_CAROUSEL_FILES} contenuti` });
      return;
    }
    ...
    const driveSource = requestedDriveSource(body);
    const driveAuthorization = await allowedPedDriveRootIds(client, rootId, driveSource);
    const metadataList = [];
    for (const fileId of fileIds) {
      const { allowed, metadata } = await allowedPedDriveFile(fileId, driveAuthorization);
      if (!allowed) {
    ...
    const format = automaticPedContentType(requestedFormat, metadataList);
    if (format === "carousel" && metadataList.length < 2) {
  ...
  if (request.method === "PATCH") {
    ...
    if (body.move_to_staging_id !== undefined) {
      const itemId = String(body.move_to_staging_id || "").trim();
      const targetRows = itemId ? await rowsByIdentifier(itemId) : [];
      if (!targetRows.length) {
        sendJson(response, 404, { error: "Contenuto PED non trovato" });
        return;
      }
      const applyEdits = body.apply_edits === true;
      if (applyEdits && String(body.caption || "").length > MAX_CAPTION_LENGTH) {
        sendJson(response, 400, { error: `Il copy non puo superare ${MAX_CAPTION_LENGTH} caratteri` });
        return;
      }
      if (applyEdits && String(body.caption_html || "").length > MAX_CAPTION_HTML_LENGTH) {
      ...
      if (applyEdits && !normalizedStatus) {
        sendJson(response, 400, { error: "publishing_status non valido" });
        return;
      }
      const isStory = targetRows[0].content_type === "story";
      const result = await supabaseFetch("/rpc/move_ped_item_to_staging", {
        method: "POST",
        body: JSON.stringify({
          p_identifier: itemId,
          p_caption: applyEdits && !isStory ? normalizedCaption(body.caption) : null,
          p_caption_html: applyEdits && !isStory ? sanitizeCaptionHtml(body.caption_html) : null,
          p_publishing_status: applyEdits ? normalizedStatus : publishingStatus(targetRows[0].publishing_status),
          p_apply_edits: applyEdits
        })
      });
      const payload = await result.json().catch(() => null);
      if (!result.ok) {
        sendJson(response, result.status, { error: payload?.message || "Impossibile spostare il contenuto tra quelli in attesa" });
        return;
      }
      const movedIdentifier = String(payload || targetRows[0].content_group_id || targetRows[0].id);
      const movedRows = await stagingRowsByIdentifier(movedIdentifier);
      if (!movedRows.length) {
        sendJson(response, 500, { error: "Contenuto spostato, ma non ricaricato dall'area in attesa" });
        return;
      }
      const client = await clientById(targetRows[0].client_id);
      sendJson(response, 200, { item: groupPedItems(movedRows, driveFolderId(client?.drive_url))[0] });
      return;
    }
    ...
    if (body.staging_id !== undefined) {
      ...
      const result = await supabaseFetch("/rpc/move_ped_staging_to_date", {
        method: "POST",
        body: JSON.stringify({ p_identifier: stagingId, p_scheduled_date: scheduledDate })
      });
      const payload = await result.json().catch(() => null);
      if (!result.ok) {
        sendJson(response, result.status, { error: payload?.message || "Impossibile inserire il contenuto nel PED" });
        return;
      }
      const movedIdentifier = String(payload || stagingRows[0].content_group_id || stagingRows[0].id);
      const insertedRows = await rowsByIdentifier(movedIdentifier);
      if (!insertedRows.length) {
        sendJson(response, 500, { error: "Contenuto programmato, ma non ricaricato dal PED" });
        return;
      }
      const client = await clientById(stagingRows[0].client_id);
    ...
    if (Array.isArray(body.carousel_member_ids)) {
      const stagingCarousel = body.staging_carousel_id !== undefined;
      const id = String(stagingCarousel ? body.staging_carousel_id : body.id || "").trim();
      const targetRows = id
        ? stagingCarousel
      ...
      const result = await supabaseFetch(stagingCarousel
        ? "/rpc/sync_ped_staging_carousel_members"
        : "/rpc/sync_ped_carousel_members", {
        method: "POST",
      ...
      const table = stagingCarousel ? "ped_staging_items" : "ped_items";
      const refreshed = await supabaseFetch(`/${table}?select=*&content_group_id=eq.${encodeURIComponent(groupId)}&order=group_position.asc`);
    ...
    if (Array.isArray(body.append_drive_file_ids)) {
      ...
      const targetRows = id
        ? stagingAppend
          ...
          : await rowsByIdentifier(id)
        : [];
      const currentGroupId = String(targetRows[0]?.content_group_id || "");
      const convertibleSingle = targetRows.length === 1
        && !currentGroupId
        && ["post", "reel"].includes(contentType(targetRows[0]?.content_type));
      const existingCarousel = Boolean(currentGroupId)
        && targetRows.every((row) => row.content_type === "carousel" && String(row.content_group_id) === currentGroupId);
      if (!targetRows.length || (!convertibleSingle && !existingCarousel)) {
        sendJson(response, 409, { error: "Puoi aggiungere contenuti soltanto a post, Reel o caroselli" });
        return;
      }
      ...
      const payload = metadataList.map((metadata) => ({
        drive_file_id: metadata.id,
        drive_file_name: metadata.name || "Contenuto Drive",
        drive_mime_type: metadata.mimeType || null,
        drive_web_url: metadata.webViewLink || `https://drive.google.com/file/d/${encodeURIComponent(metadata.id)}/view`,
      ...
      const result = await supabaseFetch(stagingAppend
        ? "/rpc/append_ped_staging_item_media"
        : "/rpc/append_ped_item_media", {
        method: "POST",
        body: JSON.stringify({
          p_identifier: id,
          p_items: payload,
      ...
      const rpcPayload = await result.json().catch(() => ({}));
      if (!result.ok) {
        const duplicate = rpcPayload?.code === "23505";
        sendJson(response, duplicate ? 409 : result.status, {
          error: duplicate
            ? stagingAppend
            ...
            : rpcPayload?.message || "Impossibile aggiornare il carosello"
        });
        return;
      }
      const groupId = String(rpcPayload || "").replace(/^"|"$/g, "");
      const table = stagingAppend ? "ped_staging_items" : "ped_items";
      const refreshed = await supabaseFetch(`/${table}?select=*&content_group_id=eq.${encodeURIComponent(groupId)}&order=group_position.asc`);
      const rows = await refreshed.json().catch(() => []);
      if (!refreshed.ok) {
        sendJson(response, refreshed.status, { error: rows.message || "Contenuti aggiunti, ma carosello non ricaricato" });
        return;
      }
      sendJson(response, 200, { item: groupPedItems(rows, rootId)[0] });
      return;
    }
    ...
    if (body.cover_frame_seconds !== undefined) {
      const coverFrameSeconds = Number(body.cover_frame_seconds);
      const effectiveType = updates.content_type || targetRows[0].content_type;
      const mimeType = String(targetRows[0].drive_mime_type || "");
      if (groupedCarousel || effectiveType !== "reel" || !mimeType.startsWith("video/")) {
        sendJson(response, 409, { error: "La copertina video può essere scelta soltanto per un Reel" });
        return;
      }
      if (!Number.isFinite(coverFrameSeconds) || coverFrameSeconds < 0 || coverFrameSeconds > 86400) {
        sendJson(response, 400, { error: "Fotogramma di copertina non valido" });
        return;
      }
      updates.cover_frame_seconds = Number(coverFrameSeconds.toFixed(3));
    }
