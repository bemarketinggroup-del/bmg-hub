...
const PROFILE_ORDER_CONCURRENCY = 6;
...
export default async function handler(request, response) {
  ...
  if (request.method === "PATCH") {
    ...
    if (Array.isArray(body.instagram_order)) {
      ...
      const targets = await mapWithConcurrency(identifiers, PROFILE_ORDER_CONCURRENCY, rowsByIdentifier);
      if (targets.some((rows) => !rows.length || rows.some((row) => String(row.client_id) !== clientId))) {
        sendJson(response, 404, { error: "Uno o piu contenuti del profilo non sono stati trovati" });
        return;
      }
      const updatedAt = new Date().toISOString();
      const results = await mapWithConcurrency(targets, PROFILE_ORDER_CONCURRENCY, async (targetRows, index) => {
        ...
        return supabaseFetch(`/ped_items?${filter}`, {
          method: "PATCH",
          body: JSON.stringify({ instagram_position: index, updated_at: updatedAt })
        });
      });
      const failed = results.find((result) => !result.ok);
      if (failed) {
        sendJson(response, failed.status, { error: "Impossibile salvare l'ordine completo del profilo" });
        return;
      }
      sendJson(response, 200, { ok: true, count: identifiers.length });
      return;
    }
