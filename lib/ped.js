...
async function allowedPedDriveRootIds(client, clientRootId, driveSource = "") {
  ...
  if (driveSource === "all") {
    const driveRoot = await driveMetadata("root");
    if (driveRoot?.id) rootIds.push(String(driveRoot.id));
  }
  return [...new Set(rootIds.filter(Boolean))];
}
...
export default async function handler(request, response) {
  ...
  if (request.method === "POST") {
    ...
    const allowedRootIds = await allowedPedDriveRootIds(client, rootId, driveSource);
    const metadataList = [];
