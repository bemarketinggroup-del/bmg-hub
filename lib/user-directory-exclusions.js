...
export function hydrateDirectoryExclusions(exclusions = [], clickUpMembers = []) {
  const members = Array.isArray(clickUpMembers) ? clickUpMembers : [];
  return (Array.isArray(exclusions) ? exclusions : []).map((item) => {
    const exclusion = normalizeDirectoryExclusion(item);
    if (!exclusion || exclusion.clickup_user_id) return exclusion;
    const member = members.find((candidate) => isDirectoryExcluded(candidate, [exclusion]));
    return member ? normalizeDirectoryExclusion({
      ...exclusion,
      clickup_user_id: member.clickup_user_id || member.id,
      full_name: exclusion.full_name || member.full_name || member.name,
      email: exclusion.email || member.email
    }) : exclusion;
  }).filter(Boolean);
}
