...
export function calendarEventBlocksStaff(event = {}, matched = []) {
  const category = clean(event.event_category || event.event_type).toLowerCase();
  if (category === "smart_working") return false;
...
async function ensureFutureSuggestions(bounds, session) {
  ...
  const [employees, rulesRows, assignments, unavailable, plans, previousRows] = await Promise.all([
  ...
  assignments.forEach((row) => counts.set(row.date, (counts.get(row.date) || 0) + 1));
  const previous = new Map();
  [...previousRows, ...assignments].forEach((row) => previous.set(row.employee_id, row.date));
  ...
  for (let week = bounds.gridStart; week < bounds.gridEnd; week = addDays(week, 7)) {
    ...
    const weekAssignments = assignments.filter((row) => row.date >= week && row.date < weekEnd);
    ...
    for (const item of allocation.assignments) {
      assignments.push({ ...item, plan_id: plan.id, status: "suggested", source: "auto" });
...
async function moveSmartAssignment(body) {
  ...
  const [rulesRows, dayAssignments, unavailable] = await Promise.all([
    tableRows("/smart_work_rules?select=max_remote_per_day&order=created_at.asc&limit=1"),
    tableRows(`/smart_work_assignments?select=id,employee_id&date=eq.${targetDate}`),
    tableRows(`/employee_unavailability?select=*&employee_id=eq.${employee.id}&date=eq.${targetDate}`)
  ]);
  const maxRemote = Math.min(MAX_SMART_PER_DAY, Math.max(1, Number(rulesRows[0]?.max_remote_per_day) || MAX_SMART_PER_DAY));
  if (dayAssignments.filter((row) => row.id !== assignment.id).length >= maxRemote) {
    throw httpError(409, `In questa giornata ci sono gia ${maxRemote} persone in smart working`, { code: "SMART_CAPACITY", can_force: false });
  }
