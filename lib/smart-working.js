...
export async function handleSmartWorking(request, response) {
  ...
  try {
    ...
    if (session.profile.role !== "admin" && !staffCanMoveSuggestion) {
    ...
    else if (action === "sync_calendar") result = await ensureCalendarRangeSynced(actionViewBounds, { force: true });
    else if (action === "sync_off_year") result = await ensureOffYearSynced(actionBounds.month.slice(0, 4), { force: body.force === true });
