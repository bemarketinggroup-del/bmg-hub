...
import {
  ...
  ensureGoogleCalendarAttendees,
  listCalendarEvents,
  updateGoogleCalendarEvent
} from "./google-calendar.js";
import { syncStaffProfilesToSmartWorking } from "./smart-working-employees.js";

const WORK_DAYS = ["mon", "tue", "wed", "thu", "fri"];
const DAY_KEYS = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"];
const SMART_EXCLUDED_NAMES = new Set(["davide de luca", "simone prezioso"]);
const STAFF_TITLE_ALIASES = new Map([
  ["andry", new Set(["andy", "andriy", "amdriy"])]
]);
const MAX_SMART_PER_DAY = 2;
const SMART_ROLLING_WEEKS = 6;
const CALENDAR_RANGE_SYNC_CACHE_MS = 5 * 60 * 1000;
const OFF_YEAR_SYNC_CACHE_MS = 5 * 60 * 1000;
const calendarRangeSyncCache = new Map();
const offYearSyncCache = new Map();
...
export function weekBounds(value = "") {
  ...
  if (Number.isNaN(parsed.getTime()) || dateOnly(parsed) !== candidate) return null;
  const weekStart = mondayOf(candidate);
  const weekEnd = addDays(weekStart, 7);
  return { weekStart, weekEnd, first: weekStart, next: weekEnd, gridStart: weekStart, gridEnd: weekEnd };
}

export function smartViewBounds(value = "", reference = new Date(), visibleWeeks = SMART_ROLLING_WEEKS) {
  const bounds = monthBounds(value);
  const referenceDate = dateOnly(reference);
  if (bounds.month !== referenceDate.slice(0, 7)) return bounds;

  const safeVisibleWeeks = Math.max(1, Number(visibleWeeks) || SMART_ROLLING_WEEKS);
  const rollingEnd = addDays(mondayOf(referenceDate), safeVisibleWeeks * 7);
  if (rollingEnd <= bounds.gridEnd) return bounds;
  return { ...bounds, gridEnd: rollingEnd, rollingFuture: true };
}

export function isClientWorkEvent(event = {}) {
  const category = clean(event.event_category || event.event_type).toLowerCase();
  if (category === "staff_leave" || isSmartCalendarEvent(event)) return false;
...
export function calendarEventBlocksStaff(event = {}, matched = []) {
  const category = clean(event.event_category || event.event_type).toLowerCase();
  if (isSmartCalendarEvent(event)) return false;
  return category === "staff_leave" || isClientWorkEvent(event) || matched.length > 0;
}

export function isSmartCalendarEvent(event = {}) {
  const category = clean(event.event_category || event.event_type).toLowerCase();
  const title = normalize(event.title || event.summary);
  return category === "smart_working"
    || /\b(smart working|smartworking|smart|lavoro da remoto|remote working)\b/.test(title);
}

export function isSmartEligibleEmployee(employee = {}) {
  return !SMART_EXCLUDED_NAMES.has(normalize(employee.full_name));
}

export function prefersFridaySmart(employee = {}) {
  return normalize(employee.full_name).split(" ")[0] === "francesco";
}
...
export function allocateWeek({ employees = [], dates = [], unavailable = new Map(), counts = new Map(), previous = new Map(), limit = 2 }) {
  ...
  sorted.forEach((employee, employeeIndex) => {
    ...
    const candidates = dates
      ...
      .sort((a, b) => {
        if (prefersFridaySmart(employee)) {
          const aFriday = dayKey(a) === "fri" ? 0 : 1;
          const bFriday = dayKey(b) === "fri" ? 0 : 1;
          if (aFriday !== bFriday) return aFriday - bFriday;
        }
...
export function buildOffCounters({ employees = [], entries = [], reviews = [], month = "" } = {}) {
  ...
  reviewByDay.forEach((status, key) => {
    if (status !== "confirmed" || uniqueDays.has(key)) return;
    const separator = key.indexOf(":");
    const employeeId = key.slice(0, separator);
    const date = key.slice(separator + 1);
    if (!WORK_DAYS.includes(dayKey(date))) return;
    uniqueDays.set(key, {
...
export function calendarOffEntries({ events = [], employees = [], rangeStart = "", rangeEnd = "" } = {}) {
  ...
  return events.flatMap((event) => {
    if (clean(event.event_category || event.event_type).toLowerCase() !== "staff_leave" || isSmartCalendarEvent(event)) return [];
...
export function calendarSmartEntries({ events = [], employees = [], rangeStart = "", rangeEnd = "" } = {}) {
  ...
  return events.flatMap((event, eventIndex) => {
    if (!isSmartCalendarEvent(event)) return [];
...
export function calendarSmartOverrides(stored = [], imported = []) {
  const importedWeeks = new Set(imported.map((row) => `${row.employee_id}:${mondayOf(row.date)}`));
  const overridden = stored.filter((row) => row.source === "auto"
    && row.status === "suggested"
    && importedWeeks.has(`${row.employee_id}:${mondayOf(row.date)}`));
  const overriddenIds = new Set(overridden.map((row) => row.id));
  return {
    assignments: stored.filter((row) => !overriddenIds.has(row.id)),
    overridden
  };
}
...
export async function handleSmartWorking(request, response) {
  ...
  try {
    ...
    const viewBounds = smartViewBounds(bounds.month);
    if (request.method === "GET") {
      sendJson(response, 200, { ...(await monthlyContext(viewBounds, session)), calendar_sync: { pending: true } }, headers);
      return;
    }
    ...
    const staffCanMoveSuggestion = action === "move_smart_assignment";
    const staffCanRefreshCalendar = ["sync_calendar", "sync_off_year", "sync_week"].includes(action);
    if (session.profile.role !== "admin" && !staffCanMoveSuggestion && !staffCanRefreshCalendar) {
    ...
    const actionViewBounds = smartViewBounds(actionBounds.month);
    const actionWeekBounds = ["sync_week", "generate_week", "approve_week"].includes(action) ? weekBounds(body.week_start) : null;
    if (["sync_week", "generate_week", "approve_week"].includes(action)) {
      if (!actionWeekBounds) throw httpError(400, "Seleziona una settimana valida");
      if (actionWeekBounds.weekStart < actionViewBounds.gridStart || actionWeekBounds.weekStart >= actionViewBounds.gridEnd) {
        throw httpError(400, "La settimana selezionata non appartiene al calendario visualizzato");
      }
    }
    let result;
    if (action === "save_rules") result = await saveRules(body);
    else if (action === "sync_calendar") result = await ensureCalendarRangeSynced(actionViewBounds, { force: session.profile.role === "admin" && body.force === true });
    else if (action === "sync_week") result = await ensureCalendarRangeSynced(actionWeekBounds, { force: session.profile.role === "admin" && body.force === true });
    else if (action === "sync_off_year") result = await ensureOffYearSynced(actionBounds.month.slice(0, 4), { force: session.profile.role === "admin" && body.force === true });
    else if (action === "set_employee_active") result = await setEmployeeActive(body);
    else if (action === "review_off_day") result = await reviewOffDay(body);
    else if (action === "ensure_future_suggestions") result = await ensureFutureSuggestions(actionViewBounds, session);
    else if (action === "move_smart_assignment") result = await moveSmartAssignment(body);
    ...
    else if (action === "generate_week") result = await generateWeek(actionWeekBounds, session);
    else if (action === "approve_week") result = await approveWeek(actionWeekBounds);
    else if (action === "generate_month") result = await generateMonth(actionBounds, session);
    else if (action === "approve_month") result = await approveMonth(actionViewBounds);
    else throw httpError(400, "Azione smart working non valida");

    sendJson(response, 200, { ok: true, result, ...(await monthlyContext(actionViewBounds, session)) }, headers);
  } catch (error) {
...
async function monthlyContext(bounds, session) {
  ...
  const canMoveSmart = ["admin", "staff"].includes(session.profile.role);
  await syncStaffProfilesToSmartWorking();
  const allEmployees = await tableRows("/smart_work_employees?select=*&order=full_name.asc");
  const employees = allEmployees.filter((employee) => employee.is_active !== false);
  const [rules, plans, assignments, unavailable, events, yearlyLeave, offReviews] = await Promise.all([
    canManage ? tableRows("/smart_work_rules?select=*&order=created_at.asc&limit=1") : Promise.resolve([]),
    ...
    canManage ? tableRows(`/employee_unavailability?select=id,employee_id,date,type,source,title,notes,google_event_id&date=gte.${yearStart}&date=lt.${yearEnd}&type=eq.staff_leave&order=date.asc`) : Promise.resolve([]),
    canManage ? tableRows(`/employee_unavailability?select=id,employee_id,date,title,updated_at&date=gte.${yearStart}&date=lt.${yearEnd}&type=eq.off_counter_review&source=eq.counter_review&order=updated_at.asc`) : Promise.resolve([])
  ]);

  const visibleEmployees = employees;
  const visibleIds = new Set(visibleEmployees.map((employee) => employee.id));
  const visibleSmartEmployees = visibleEmployees.filter(isSmartEligibleEmployee);
  const visibleSmartIds = new Set(visibleSmartEmployees.map((employee) => employee.id));
  const visibleAssignments = assignments.filter((row) => visibleSmartIds.has(row.employee_id));
  ...
  }).filter((row) => visibleSmartIds.has(row.employee_id));
  const gridDates = [];
  ...
  return {
    ...
    rolling_future: bounds.rollingFuture === true,
    grid_dates: gridDates,
    rules: canManage ? rules[0] || { max_remote_per_day: 2, remote_days_per_employee: 1, working_days: WORK_DAYS } : null,
    staff: visibleEmployees,
    smart_staff: visibleSmartEmployees,
    all_staff: canManage ? allEmployees : visibleEmployees,
    plans,
    ...
    off_counters: canManage ? buildOffCounters({
      employees: visibleEmployees,
      entries: yearlyLeave,
      reviews: offReviews.filter((row) => visibleIds.has(row.employee_id)),
      month: bounds.month
    }) : { month: bounds.month, year, month_total: 0, year_total: 0, staff: [] },
    can_manage: canManage,
    can_move_smart: canMoveSmart
  };
}

async function setEmployeeActive(body) {
  const employeeId = clean(body.employee_id);
  if (!employeeId || typeof body.is_active !== "boolean") throw httpError(400, "Stato della persona non valido");
  const employee = (await tableRows(`/smart_work_employees?select=*&id=eq.${encodeURIComponent(employeeId)}&limit=1`))[0];
  if (!employee) throw httpError(404, "Persona non trovata nei turni");

  if (body.is_active === false) {
    const today = dateOnly(new Date());
    const futureSuggestions = await tableRows(`/smart_work_assignments?select=id&employee_id=eq.${encodeURIComponent(employeeId)}&date=gte.${today}&source=eq.auto&status=eq.suggested`);
    for (const assignment of futureSuggestions) {
      await deleteWhere(`/smart_work_assignments?id=eq.${encodeURIComponent(assignment.id)}`);
    }
  }

  const updated = await writeRows(`/smart_work_employees?id=eq.${encodeURIComponent(employeeId)}`, {
    is_active: body.is_active,
    updated_at: new Date().toISOString()
  }, "return=representation", "PATCH");
  return { employee: updated[0] || { ...employee, is_active: body.is_active } };
}
...
async function syncCalendar(bounds) {
  ...
  for (const originalEvent of events) {
    ...
    const matched = matchedEmployees(event, employees);
    const missingAttendeeEmails = matched
      .map((employee) => clean(employee.email).toLowerCase())
      .filter((email) => email && !(event.attendees || []).some((attendee) => normalize(attendee.email) === normalize(email)));
    if (missingAttendeeEmails.length && calendarEventCanInvite(event)) {
      try {
        const attendeeSync = await ensureGoogleCalendarAttendees(event.id, missingAttendeeEmails);
        event = attendeeSync.event;
        invited += attendeeSync.added.length;
      } catch {
        invitationErrors += missingAttendeeEmails.length;
      }
    }
    processedEvents.push({ event, matched });
  }
  ...
  const calendarId = process.env.GOOGLE_CALENDAR_ID || "beviralagency@gmail.com";
  const cacheRows = processedEvents.map(({ event }) => {
  ...
  const storedCache = cacheRows.length
    ? await writeRows("/calendar_events_cache?on_conflict=google_event_id,calendar_id", cacheRows, "resolution=merge-duplicates,return=representation")
    : [];
  const cacheByEventId = new Map(storedCache.map((row) => [row.google_event_id, row]));
  const attendeeRows = [];
  const unavailableRows = [];

  processedEvents.forEach(({ event, matched }) => {
    const cacheRow = cacheByEventId.get(event.id);
    if (!cacheRow) return;
    ...
    if (!calendarEventBlocksStaff(event, matched)) return;
  ...
  if (attendeeRows.length) await writeRows("/calendar_event_attendees", attendeeRows);
  if (unavailableRows.length) await writeRows("/employee_unavailability", unavailableRows);
  return { cached: storedCache.length, blocked: unavailableRows.length, invited, invitation_errors: invitationErrors };
}

function calendarEventCanInvite(event) {
  const today = dateOnly(new Date());
  if (event.all_day) return /^\d{4}-\d{2}-\d{2}$/.test(clean(event.end_at)) && clean(event.end_at) > today;
  const endAt = new Date(clean(event.end_at || event.start_at));
  return !Number.isNaN(endAt.valueOf()) && endAt.valueOf() > Date.now();
}
...
async function syncOffYear(year) {
  ...
  const rows = entries.map((entry) => ({
    ...
    google_event_id: entry.source_event_id || null,
    title: entry.title,
    notes: entry.notes || "staff_leave",
    is_manual: false,
    forced: false,
    updated_at: new Date().toISOString()
  }));

  await deleteWhere(`/employee_unavailability?source=eq.google_calendar&type=eq.staff_leave&date=gte.${yearStart}&date=lt.${yearEnd}`);
  if (rows.length) await writeRows("/employee_unavailability", rows);
  return {
    year,
    calendar_events: events.length,
    off_events: new Set(entries.map((entry) => entry.source_event_id).filter(Boolean)).size,
    off_days: rows.length
  };
}

async function ensureOffYearSynced(year, { force = false } = {}) {
  if (!/^\d{4}$/.test(clean(year))) throw httpError(400, "Anno del contatore OFF non valido");
  const cacheKey = `${process.env.GOOGLE_CALENDAR_ID || "beviralagency@gmail.com"}:${year}`;
  const cached = offYearSyncCache.get(cacheKey);
  if (!force && cached?.result && Date.now() - cached.createdAt < OFF_YEAR_SYNC_CACHE_MS) {
    return { ...cached.result, cached_year: true };
  }
  if (cached?.promise) return cached.promise;

  const promise = syncOffYear(year)
    .then((result) => {
      offYearSyncCache.set(cacheKey, { createdAt: Date.now(), result });
      return { ...result, cached_year: false };
    })
    .catch((error) => {
      offYearSyncCache.delete(cacheKey);
      throw error;
    });
  offYearSyncCache.set(cacheKey, { createdAt: Date.now(), promise });
  return promise;
}
...
async function ensureFutureSuggestions(bounds, session) {
  const currentWeek = mondayOf(new Date());
  const [employees, rulesRows, assignments, unavailable, plans, previousRows, calendarEvents] = await Promise.all([
    tableRows("/smart_work_employees?select=*&is_active=eq.true&order=full_name.asc"),
    tableRows("/smart_work_rules?select=*&order=created_at.asc&limit=1"),
    tableRows(`/smart_work_assignments?select=*&date=gte.${bounds.gridStart}&date=lt.${bounds.gridEnd}&order=date.asc`),
    tableRows(`/employee_unavailability?select=*&date=gte.${bounds.gridStart}&date=lt.${bounds.gridEnd}`),
    tableRows(`/smart_work_plans?select=*&week_start_date=gte.${bounds.gridStart}&week_start_date=lt.${bounds.gridEnd}`),
    tableRows(`/smart_work_assignments?select=employee_id,date&date=gte.${addDays(bounds.gridStart, -7)}&date=lt.${bounds.gridStart}`),
    tableRows(`/calendar_events_cache?select=*&start_at=lt.${encodeURIComponent(`${bounds.gridEnd}T00:00:00Z`)}&end_at=gt.${encodeURIComponent(`${addDays(bounds.gridStart, -7)}T00:00:00Z`)}&order=start_at.asc`)
  ]);
  const smartEmployees = employees.filter(isSmartEligibleEmployee);
  const importedSmart = calendarSmartEntries({
    events: calendarEvents.map(cachedCalendarEvent),
    employees: smartEmployees,
    rangeStart: addDays(bounds.gridStart, -7),
    rangeEnd: bounds.gridEnd
  });
  const currentImportedSmart = importedSmart.filter((row) => row.date >= bounds.gridStart);
  const reconciled = calendarSmartOverrides(assignments, currentImportedSmart);
  for (const row of reconciled.overridden) await deleteWhere(`/smart_work_assignments?id=eq.${row.id}`);
  const effectiveAssignments = mergeSmartAssignments(reconciled.assignments, currentImportedSmart);
  const rules = rulesRows[0] || { max_remote_per_day: MAX_SMART_PER_DAY };
  ...
  effectiveAssignments.forEach((row) => counts.set(row.date, (counts.get(row.date) || 0) + 1));
  const previous = new Map();
  [...previousRows, ...importedSmart, ...effectiveAssignments].forEach((row) => previous.set(row.employee_id, row.date));

  let created = 0;
  const conflicts = [];
  for (let week = bounds.gridStart; week < bounds.gridEnd; week = addDays(week, 7)) {
    if (week <= currentWeek || plans.some((plan) => plan.week_start_date === week && plan.status === "approved")) continue;
    const dates = Array.from({ length: 5 }, (_, index) => addDays(week, index));
    const weekEnd = addDays(week, 7);
    const weekAssignments = effectiveAssignments.filter((row) => row.date >= week && row.date < weekEnd);
    const assignedIds = new Set(weekAssignments.map((row) => row.employee_id));
    const eligible = smartEmployees.filter((employee) => !assignedIds.has(employee.id));
    ...
    const allocation = allocateWeek({
      ...
      counts,
      previous,
      limit: rules.max_remote_per_day
    });
    conflicts.push(...allocation.conflicts.map((conflict) => ({ ...conflict, week_start: week })));
    const plan = await ensureDraftPlan(week, session.profile.id);
    const suggestionRows = allocation.assignments.map((item) => ({
      plan_id: plan.id,
      employee_id: item.employee_id,
      date: item.date,
      status: "suggested",
      reason: "Proposta automatica per completare la rotazione settimanale.",
      google_event_id: null,
      forced: false,
      source: "auto",
      updated_at: new Date().toISOString()
    }));
    if (suggestionRows.length) await writeRows("/smart_work_assignments", suggestionRows);
    for (const item of allocation.assignments) {
      effectiveAssignments.push({ ...item, plan_id: plan.id, status: "suggested", source: "auto" });
      previous.set(item.employee_id, item.date);
      created += 1;
    }
  }
  return { created, overridden: reconciled.overridden.length, conflicts };
}

async function moveSmartAssignment(body) {
  const assignmentId = clean(body.assignment_id);
  const targetDate = validWorkDate(body.target_date);
  const forced = body.force === true;
  if (!assignmentId || !targetDate) throw httpError(400, "Spostamento smart non valido");

  const assignment = (await tableRows(`/smart_work_assignments?select=*&id=eq.${encodeURIComponent(assignmentId)}&source=eq.auto&status=eq.suggested&limit=1`))[0];
  if (!assignment) throw httpError(404, "Proposta smart non trovata");
  const currentWeek = mondayOf(new Date());
  if (mondayOf(assignment.date) < currentWeek || mondayOf(targetDate) !== mondayOf(assignment.date) || targetDate < dateOnly(new Date())) {
    throw httpError(409, "Le proposte si possono spostare tra i giorni disponibili della settimana corrente o di una settimana futura");
  }
  if (targetDate === assignment.date) return assignment;

  const employee = (await tableRows(`/smart_work_employees?select=*&id=eq.${assignment.employee_id}&is_active=eq.true&limit=1`))[0];
  if (!employee) throw httpError(404, "Dipendente non trovato");
  if (!isSmartEligibleEmployee(employee)) throw httpError(409, `${employee.full_name} e escluso dalla rotazione smart working`);
  const unavailable = await tableRows(`/employee_unavailability?select=*&employee_id=eq.${employee.id}&date=eq.${targetDate}`);
  const leave = unavailable.filter((row) => row.type === "staff_leave");
  if (leave.length) throw httpError(409, `${employee.full_name} risulta OFF in questa giornata`, { code: "STAFF_LEAVE_CONFLICT", can_force: false });
  const busy = unavailable.filter((row) => row.type === "client_commitment");
  if (busy.length && !forced) throw httpError(409, `${employee.full_name} ha gia un appuntamento in calendario`, {
    code: "CALENDAR_CONFLICT",
    can_force: true,
    conflicts: busy.map(conflictSummary)
  });

  return writeRows(`/smart_work_assignments?id=eq.${assignment.id}`, {
    date: targetDate,
    forced: forced && busy.length > 0,
    reason: forced && busy.length
      ? `Proposta spostata manualmente nonostante: ${busy.map((item) => item.title).join(", ")}`
      : "Proposta spostata manualmente tramite calendario.",
    updated_at: new Date().toISOString()
  }, "return=representation", "PATCH");
}
...
async function saveManualEntry(body, session) {
  ...
  if (kind === "smart" && !isSmartEligibleEmployee(employee)) throw httpError(409, `${employee.full_name} e escluso dalla rotazione smart working`);
...
async function generateWeek(bounds, session) {
  const calendarRangeStart = addDays(bounds.weekStart, -7);
  await syncCalendar({ gridStart: calendarRangeStart, gridEnd: bounds.weekEnd });
  const [employees, rulesRows, weekAssignments, unavailable, calendarEvents, previousRows] = await Promise.all([
    tableRows("/smart_work_employees?select=*&is_active=eq.true&order=full_name.asc"),
    tableRows("/smart_work_rules?select=*&order=created_at.asc&limit=1"),
    tableRows(`/smart_work_assignments?select=*&date=gte.${bounds.weekStart}&date=lt.${bounds.weekEnd}`),
    tableRows(`/employee_unavailability?select=*&date=gte.${calendarRangeStart}&date=lt.${bounds.weekEnd}`),
    tableRows(`/calendar_events_cache?select=*&start_at=lt.${encodeURIComponent(`${bounds.weekEnd}T00:00:00Z`)}&end_at=gt.${encodeURIComponent(`${calendarRangeStart}T00:00:00Z`)}&order=start_at.asc`),
    tableRows(`/smart_work_assignments?select=employee_id,date&date=gte.${calendarRangeStart}&date=lt.${bounds.weekStart}`)
  ]);
  const smartEmployees = employees.filter(isSmartEligibleEmployee);
  const importedSmart = calendarSmartEntries({
    events: calendarEvents.map(cachedCalendarEvent),
    ...
    rangeEnd: bounds.weekEnd
  });

  const autoRows = weekAssignments.filter((row) => row.source === "auto");
  const replacedGoogleEventIds = new Set(autoRows.map((row) => clean(row.google_event_id)).filter(Boolean));
  ...
  const manualRows = mergeSmartAssignments(
    weekAssignments.filter((row) => row.source !== "auto"),
    importedSmart.filter((row) => row.date >= bounds.weekStart && !replacedGoogleEventIds.has(clean(row.google_event_id)))
  );
  const counts = new Map();
  ...
  const previous = new Map();
  [...previousRows, ...importedSmart.filter((row) => row.date < bounds.weekStart && !replacedGoogleEventIds.has(clean(row.google_event_id)))]
    .forEach((row) => previous.set(row.employee_id, row.date));

  const assignedIds = new Set(manualRows.map((row) => row.employee_id));
  const eligible = smartEmployees.filter((employee) => !assignedIds.has(employee.id));
  const dates = Array.from({ length: 5 }, (_, index) => addDays(bounds.weekStart, index));
  ...
  const plan = allocation.assignments.length || manualRows.length
    ? await ensureDraftPlan(bounds.weekStart, session.profile.id)
    : null;
  ...
  for (const item of allocation.assignments) {
    await writeRows("/smart_work_assignments", {
      ...
      date: item.date,
      status: "suggested",
      reason: `Proposta settimanale per evitare gli impegni cliente e non superare ${Math.min(MAX_SMART_PER_DAY, Math.max(1, Number(rules.max_remote_per_day) || MAX_SMART_PER_DAY))} persone in smart.`,
      google_event_id: null,
      forced: false,
      source: "auto",
      updated_at: new Date().toISOString()
  ...
  return {
    ...
    week_end: addDays(bounds.weekEnd, -1),
    created: allocation.assignments.length,
    conflicts: allocation.conflicts
  };
}

async function approveWeek(bounds) {
  const plans = await tableRows(`/smart_work_plans?select=id&week_start_date=eq.${bounds.weekStart}&status=eq.draft`);
  const result = await approvePlans(plans);
  return { week_start: bounds.weekStart, week_end: addDays(bounds.weekEnd, -1), ...result };
}

async function generateMonth(bounds, session) {
  await syncCalendar(bounds);
  const calendarRangeStart = addDays(bounds.first, -7);
  const [employees, rulesRows, allAssignments, unavailable, calendarEvents] = await Promise.all([
    tableRows("/smart_work_employees?select=*&is_active=eq.true&order=full_name.asc"),
    tableRows("/smart_work_rules?select=*&order=created_at.asc&limit=1"),
    tableRows(`/smart_work_assignments?select=*&date=gte.${bounds.first}&date=lt.${bounds.next}`),
    tableRows(`/employee_unavailability?select=*&date=gte.${calendarRangeStart}&date=lt.${bounds.next}`),
    tableRows(`/calendar_events_cache?select=*&start_at=lt.${encodeURIComponent(`${bounds.next}T00:00:00Z`)}&end_at=gt.${encodeURIComponent(`${calendarRangeStart}T00:00:00Z`)}&order=start_at.asc`)
  ]);
  const smartEmployees = employees.filter(isSmartEligibleEmployee);
  const importedSmart = calendarSmartEntries({
    events: calendarEvents.map(cachedCalendarEvent),
  ...
  const manualRows = mergeSmartAssignments(
    allAssignments.filter((row) => row.source !== "auto"),
    importedSmart.filter((row) => row.date >= bounds.first)
  );
  const counts = new Map();
  manualRows.forEach((row) => counts.set(row.date, (counts.get(row.date) || 0) + 1));
  const blocked = new Map();
  ...
  [...previousRows, ...importedSmart.filter((row) => row.date < bounds.first)].forEach((row) => previous.set(row.employee_id, row.date));
  ...
  for (let week = mondayOf(bounds.first); week < bounds.next; week = addDays(week, 7)) {
    ...
    const eligible = smartEmployees.filter((employee) => !already.has(employee.id));
    const allocation = allocateWeek({ employees: eligible, dates, unavailable: blocked, counts, previous, limit: rules.max_remote_per_day });
...
async function approveMonth(bounds) {
  ...
  return approvePlans(plans);
}

async function approvePlans(plans) {
  const employees = await tableRows("/smart_work_employees?select=*&is_active=eq.true&order=full_name.asc");
  let published = 0;
...
export function matchedEmployees(event, employees) {
  ...
  const allowFuzzyAbbreviation = ["staff_leave", "smart_working"].includes(
    clean(event.event_category || event.event_type).toLowerCase()
  ) || isSmartCalendarEvent(event);
  ...
  titleTokens.forEach((token) => {
    ...
    const candidates = employees.filter((employee) => {
      ...
      return firstName.length >= 4 && (
        ...
        || (allowFuzzyAbbreviation && commonPrefixLength(token, firstName) >= 4)
      );
    });
  ...
  return employees.filter((employee) => {
    ...
    if ([...(STAFF_TITLE_ALIASES.get(firstName) || [])].some((alias) => titleTokens.has(alias))) return true;
