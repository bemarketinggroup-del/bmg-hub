...
export function buildOffCounters({ employees = [], entries = [], month = "" } = {}) {
  ...
  entries.forEach((entry) => {
    ...
    uniqueDays.set(`${entry.employee_id}:${date}`, { employee_id: entry.employee_id, date });
  });

  const staff = [...employees]
    .sort((a, b) => clean(a.full_name).localeCompare(clean(b.full_name), "it"))
    .map((employee) => {
      const dates = [...uniqueDays.values()].filter((entry) => entry.employee_id === employee.id);
      return {
        employee_id: employee.id,
        month_days: dates.filter((entry) => entry.date.startsWith(`${selectedMonth}-`)).length,
        year_days: dates.length
      };
    });
...
export async function handleSmartWorking(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      sendJson(response, 200, await monthlyContext(bounds, session), headers);
      return;
    }
    ...
    else if (action === "sync_calendar") result = await syncCalendar(bounds);
    ...
    else if (action === "generate_month") result = await generateMonth(bounds, session);
    else if (action === "approve_month") result = await approveMonth(bounds);
    else throw httpError(400, "Azione smart working non valida");

    const freshBounds = monthBounds(body.month || bounds.month);
    sendJson(response, 200, { ok: true, result, ...(await monthlyContext(freshBounds, session)) }, headers);
  } catch (error) {
...
async function monthlyContext(bounds, session) {
  ...
  const [rules, plans, assignments, unavailable, events, yearlyLeave, calendarYearlyLeave] = await Promise.all([
    ...
    tableRows(`/employee_unavailability?select=employee_id,date,type,source&date=gte.${yearStart}&date=lt.${yearEnd}&type=eq.staff_leave&order=date.asc`),
  ...
  return {
    ...
    plans,
    assignments: assignments.filter((row) => visibleIds.has(row.employee_id)),
    ...
    off_counters: buildOffCounters({ employees: visibleEmployees, entries: [...yearlyLeave, ...calendarYearlyLeave], month: bounds.month }),
