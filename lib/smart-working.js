...
async function monthlyContext(bounds, session) {
  const [employees, rules, plans, assignments, unavailable, events] = await Promise.all([
...
function matchedEmployees(event, employees) {
  ...
  return employees.filter((employee) => {
    ...
    return firstName.length >= 3 && firstNameCounts.get(firstName) === 1 && titleTokens.has(firstName);
  });
}
