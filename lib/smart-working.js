...
export async function handleSmartWorking(request, response) {
  ...
  try {
    ...
    const staffCanRefreshCalendar = ["sync_calendar", "sync_off_year"].includes(action);
    ...
    const actionWeekBounds = ["generate_week", "approve_week"].includes(action) ? weekBounds(body.week_start) : null;
    if (["generate_week", "approve_week"].includes(action)) {
...
async function moveSmartAssignment(body) {
  ...
  const currentWeek = mondayOf(new Date());
  if (mondayOf(assignment.date) <= currentWeek || mondayOf(targetDate) !== mondayOf(assignment.date)) {
    throw httpError(409, "Le proposte si possono spostare solo tra i giorni della stessa settimana futura");
  }
