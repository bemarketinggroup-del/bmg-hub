...
export async function handleSmartWorking(request, response) {
  ...
  try {
    ...
    if (request.method === "GET") {
      sendJson(response, 200, { ...(await monthlyContext(bounds, session)), calendar_sync: { pending: true } }, headers);
      return;
    }
    ...
    else if (action === "ensure_future_suggestions") result = await ensureFutureSuggestions(actionBounds, session);
