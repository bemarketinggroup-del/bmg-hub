...
import { isOperationalTeamTask } from "./clickup-task-access.js";
import { ensureAiBudgetAvailable, estimateOpenAiCost } from "./ai-budget.js";
...
function safeProviderError(value) {
  return compactText(value, 320).replace(/sk-[a-z0-9_-]+/gi, "[redacted]");
}
...
export function deterministicClientMatch(task, clients) {
  const haystack = ` ${normalize([task.name, task.description, (task.tags || []).join(" ")].filter(Boolean).join(" "))} `;
  const matches = [];
  for (const client of clients) {
    const terms = [...new Set([client.name, ...(client.aliases || []).map((alias) => alias.alias)].flatMap((term) => {
      const normalized = normalize(term);
      if (!normalized) return [];
      const distinctiveWords = normalized.split(" ")
        .filter((word) => word.length >= 4 && !GENERIC_CLIENT_TERMS.has(word));
      return [normalized, ...distinctiveWords];
    }))];
    const found = terms
      .filter((term) => haystack.includes(` ${term} `))
      .sort((left, right) => right.split(" ").length - left.split(" ").length || right.length - left.length)[0];
    if (found) {
...
async function openaiJson(input, schema, name) {
  ...
  if (!budgetCheck.allowed) {
    return { ok: false, status: 429, error: budgetCheck.error, budget: budgetCheck.budget };
  }
  ...
  if (!response.ok) {
    console.error("OpenAI task assist request failed", {
      status: response.status,
      type: safeProviderError(data?.error?.type),
      code: safeProviderError(data?.error?.code),
      param: safeProviderError(data?.error?.param),
      message: safeProviderError(data?.error?.message)
    });
    const error = data?.error?.code === "credit_balance_exhausted" || data?.error?.type === "insufficient_quota"
      ...
      : response.status === 401 || response.status === 403
        ? "La configurazione AI deve essere aggiornata dall'amministratore."
        : "Il servizio AI non ha accettato la richiesta. Riprova tra poco.";
    return { ok: false, status: response.status, error };
  }
  const text = data.output_text || data.output?.flatMap((item) => item.content || []).find((item) => item.type === "output_text")?.text || "";
  try {
    return { ok: true, data: JSON.parse(text), billing: estimateOpenAiCost(OPENAI_MODEL, data.usage || {}) };
  } catch {
...
async function aiClientMatch(task, clients) {
  ...
  return { ...result.data, source: "ai", billing: result.billing };
}
...
async function analyzeMissingClients(session) {
  ...
  for (const task of tasks) {
    ...
    await audit(session, "analyze_client_tag", safe.action === "unresolved" ? "unresolved" : (safe.action === "auto_apply" ? "suggestion" : "suggestion"), {
      ...
      metadata: { action: safe.action, source: safe.source, ...(match?.billing || {}) }
    });
  }
...
async function improveDescription(session, body) {
  const taskId = clean(body.clickup_task_id || body.task_id);
  const rows = taskId ? await taskRows(`clickup_task_id=eq.${encodeURIComponent(taskId)}&limit=1`) : [];
  if (!rows.length && taskId) {
    rows.push(...await taskRows(`id=eq.${encodeURIComponent(taskId)}&limit=1`));
  }
  const isDraft = !taskId;
  const task = rows[0] || (isDraft ? {
    name: compactText(body.name, 220),
    description: compactText(body.description, 1800),
    client_tag: clean(body.client_tag),
    status: clean(body.status) || "to do",
    priority: clean(body.priority)
  } : null);
  if (!task) return { status: 404, body: { error: "Task non trovata" } };
  if (isDraft && !task.name && !task.description) {
    return { status: 400, body: { error: "Scrivi prima il titolo o la descrizione della task" } };
  }
  if (!isDraft && session.profile.role === "staff" && !isOperationalTeamTask(task)) {
    return { status: 403, body: { error: "Puoi usare AI solo sulle task operative del team" } };
  }
  ...
  if (!result.ok) {
    await audit(session, "improve_description", "error", { taskId, metadata: { reason: result.error, draft: isDraft } });
    return { status: result.status || 503, body: { error: result.error } };
  }
  await audit(session, "improve_description", "suggestion", { taskId, confidence: result.data.confidence, metadata: { model: OPENAI_MODEL, draft: isDraft, ...(result.billing || {}) } });
