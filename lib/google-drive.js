...
// Hub writes invalidate this cache, so a longer TTL makes repeated navigation
// fast without hiding uploads, renames or deletions performed in the app.
const FOLDER_TTL = 2 * 60 * 1000;
...
export async function googleDriveAccessToken() {
  ...
  if (!response.ok || !data.access_token) {
    throw googleDriveAuthError({
      message: data.error_description || data.error || "Autenticazione Google Drive non riuscita",
...
export async function googleDriveWriteAccessToken() {
  ...
  if (!response.ok || !data.access_token) {
    throw googleDriveAuthError({
      message: data.error_description || data.error || "Autorizzazione Google Drive upload non riuscita",
      status: response.status,
      reason: data.error,
      authSource: "dedicated_oauth"
    });
  }
...
export async function googleDriveHealth() {
  const [read, write] = await Promise.all([
    googleDriveReadHealth(),
    googleDriveWriteHealth()
  ]);
  const online = read.status === "online" && write.status === "online";
  return {
    status: online ? "online" : "degraded",
    code: online ? "ok" : read.status !== "online" ? read.code : write.code,
...
async function googleDriveReadHealth() {
  if (!googleDriveConfigured()) {
    return driveHealthFailure("drive_read_not_configured", "service_account");
  }
  try {
    const response = await driveRequest("/files?pageSize=1&fields=files(id)&spaces=drive");
    if (!response.ok) throw await googleDriveResponseError(response, "Google Drive lettura non autorizzata", "service_account");
    return { status: "online", code: "ok", auth_source: "service_account" };
  } catch (error) {
    return driveHealthFailure(googleDriveErrorCode(error, "read"), error?.authSource || "service_account");
  }
}

async function googleDriveWriteHealth() {
  if (!googleDriveWriteConfigured()) {
    return driveHealthFailure("drive_write_not_configured", "dedicated_oauth");
  }
  try {
    const response = await driveWriteRequest("/about?fields=user(permissionId)");
    if (!response.ok) throw await googleDriveResponseError(response, "Google Drive scrittura non autorizzata", "dedicated_oauth");
    return { status: "online", code: "ok", auth_source: "dedicated_oauth" };
  } catch (error) {
    return driveHealthFailure(googleDriveErrorCode(error, "write"), error?.authSource || "dedicated_oauth");
  }
}

function driveHealthFailure(code, authSource) {
  return { status: "offline", code, auth_source: authSource };
}

async function googleDriveResponseError(response, fallback, authSource) {
  const data = await response.json().catch(() => ({}));
  const reason = data.error?.errors?.[0]?.reason || data.error?.status || data.error;
  return googleDriveAuthError({
...
function googleDriveAuthError({ message, status, reason, authSource }) {
...
function googleDriveErrorCode(error, capability) {
  const reason = String(error?.reason || "");
  if (["invalid_client", "invalid_grant"].includes(reason)) return `drive_${capability}_oauth_invalid`;
  if ([401, 403].includes(Number(error?.status))) return `drive_${capability}_not_authorized`;
  return `drive_${capability}_unavailable`;
}
...
export async function driveWriteRequest(path, options = {}) {
  return driveExternalRequestWithWriteAccess(`${DRIVE_API}${path}`, options);
}

export async function driveExternalRequestWithWriteAccess(url, options = {}) {
...
export async function findDriveFolderWithWriteAccess({ parentId, name }) {
  const safeParentId = String(parentId || "root").replace(/'/g, "\\'");
  ...
  const response = await driveWriteRequest(`/files?${params}`);
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive folder search error ${response.status}`);
  return data.files?.[0] || null;
}

export async function listDriveFoldersWithWriteAccess({ parentId = "root" } = {}) {
  const safeParentId = String(parentId || "root").replace(/'/g, "\\'");
  const query = [
    `'${safeParentId}' in parents`,
    "mimeType = 'application/vnd.google-apps.folder'",
  ...
  do {
    ...
    const response = await driveWriteRequest(`/files?${params}`);
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data.error?.message || `Google Drive folder list error ${response.status}`);
...
export async function ensureDriveFolderWithWriteAccess({ parentId = "root", name }) {
  const existing = await findDriveFolderWithWriteAccess({ parentId, name });
  return existing || createDriveFolder({ parentId, name });
}

export async function ensureDriveServiceAccountPermission(fileId, role = "writer") {
  const serviceAccountEmail = parseServiceAccount()?.client_email;
  if (!serviceAccountEmail) throw new Error("Email del service account Google Drive non configurata");

  const permissionParams = new URLSearchParams({
    fields: "permissions(id,type,role,emailAddress)",
    supportsAllDrives: "true"
  });
  const permissionResponse = await driveWriteRequest(`/files/${encodeURIComponent(fileId)}/permissions?${permissionParams}`);
  const permissionData = await permissionResponse.json().catch(() => ({}));
  if (!permissionResponse.ok) {
    throw new Error(permissionData.error?.message || `Google Drive permissions error ${permissionResponse.status}`);
  }
  const existing = (permissionData.permissions || []).find((permission) => (
    String(permission.emailAddress || "").toLowerCase() === serviceAccountEmail.toLowerCase()
  ));
  ...
  const response = await driveWriteRequest(`/files/${encodeURIComponent(fileId)}/permissions?${createParams}`, {
    method: "POST",
    headers: { "Content-Type": "application/json; charset=utf-8" },
    body: JSON.stringify({
      type: "user",
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive share error ${response.status}`);
  clearDriveCaches();
  return data;
}
...
export async function moveDriveFile({ fileId, targetFolderId, removeParentIds = [] }) {
  const fields = "id,name,mimeType,parents,modifiedTime,webViewLink";
  const params = new URLSearchParams({
    supportsAllDrives: "true",
    fields,
  ...
  const parentsToRemove = [...new Set(removeParentIds.map(String).filter(Boolean))]
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive move error ${response.status}`);
  clearDriveCaches();
  return data;
}
...
export async function driveMetadataWithWriteAccess(fileId) {
  const fields = "id,name,mimeType,parents,size,modifiedTime,hasThumbnail,thumbnailLink,webViewLink";
  const response = await driveWriteRequest(`/files/${encodeURIComponent(fileId)}?fields=${encodeURIComponent(fields)}&supportsAllDrives=true`);
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive error ${response.status}`);
  return data;
}
...
export async function isInsideDriveRootWithWriteAccess(fileId, rootId, initialMetadata = null) {
  if (!fileId || !rootId) return false;
  if (fileId === rootId) return true;
  ...
  for (let depth = 0; pending.length && depth < 20; depth += 1) {
    ...
    for (const id of pending) {
      if (visited.has(id)) continue;
      visited.add(id);
      const metadata = id === fileId && initialMetadata
        ? initialMetadata
        : await driveMetadataWithWriteAccess(id);
      for (const parentId of metadata.parents || []) {
...
export async function listDriveFolder(folderId, { fresh = false } = {}) {
  ...
  const page = await listDriveFolderPage(folderId, { fresh, pageSize: 200 });
  return storeValue(folderCache, folderId, page.files, FOLDER_TTL);
}

export async function listDriveFolderPage(folderId, { fresh = false, pageToken = "", pageSize = 60 } = {}) {
  const normalizedPageSize = Math.max(1, Math.min(1000, Number(pageSize) || 60));
  const cacheKey = `${folderId}:${pageToken || "first"}:${normalizedPageSize}`;
  if (!fresh) {
    const cached = cachedValue(folderPageCache, cacheKey);
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive error ${response.status}`);
  const page = { files: data.files || [], nextPageToken: String(data.nextPageToken || ""), cacheStatus: "miss" };
  storeValue(folderPageCache, cacheKey, page, FOLDER_TTL);
  return page;
}

export async function listDriveFolderPageWithWriteAccess(folderId, { pageToken = "", pageSize = 60 } = {}) {
  const normalizedPageSize = Math.max(1, Math.min(1000, Number(pageSize) || 60));
  const query = `'${String(folderId).replace(/'/g, "\\'")}' in parents and trashed = false`;
  const fields = "nextPageToken,files(id,name,mimeType,parents,modifiedTime,size,thumbnailLink,webViewLink)";
  const params = new URLSearchParams({
    q: query,
    fields,
    pageSize: String(normalizedPageSize),
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error?.message || `Google Drive error ${response.status}`);
  return { files: data.files || [], nextPageToken: String(data.nextPageToken || ""), cacheStatus: "oauth" };
}

export function clearDriveFolderCache(folderId) {
  const normalized = String(folderId || "");
  folderCache.delete(normalized);
  for (const key of folderPageCache.keys()) {
    if (key.startsWith(`${normalized}:`)) folderPageCache.delete(key);
  }
}
