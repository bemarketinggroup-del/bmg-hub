...
async function validateDriveSelection({ clientId, sourceLibrary, sourceFolderId, files }) {
  const clientResult = await supabaseFetch(
    `/clients?select=id,name,drive_url,notes&id=eq.${encodeURIComponent(clientId)}&limit=1`
  );
  ...
  if (sourceLibrary) {
    const libraries = await resolveClientDriveLibraries(client.name, listDriveFolder, clientConnectionSettings(client.notes));
...
async function notifyGraphicTeam(sender, client, review) {
  ...
  const recipients = profiles
    ...
    .filter((item) => item.active !== false && String(item.id) !== String(sender.id) && canAccessModule(item, "graphics") && isGraphicDesigner(item));
...
async function loadProfiles() {
  const result = await supabaseFetch(
    "/staff_profiles?select=id,full_name,email,role,professional_role,professional_role_label,active,module_permissions&order=full_name.asc"
  );
  return result.ok ? result.json() : [];
}
...
function mediaFile(review, file) {
  ...
  const mediaUrl = (action) => {
    const media = action === "thumbnail" ? null : {
      mimeType: file.mime_type || "application/octet-stream",
      name: file.name || "file"
    };
    const token = signDriveMediaToken({ clientId, rootId, fileId: file.id, action, media });
