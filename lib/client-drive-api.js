...
export default async function handler(request, response) {
  ...
  if (request.method === "GET" && mediaToken && ["content", "thumbnail"].includes(action)) {
    let tokenData = null;
    try {
    ...
    try {
      if (action === "content") await sendFile(request, response, url, tokenData.rid, { trusted: true });
      else await sendThumbnail(response, url, tokenData.rid, { trusted: true });
    } catch (error) {
...
async function createUploadSession(request, response, rootId) {
  ...
  const uploadUrl = await createDriveUploadSession({ folderId, name, mimeType, size });
...
async function sendFile(request, response, url, rootId, { trusted = false } = {}) {
  const fileId = String(url.searchParams.get("file_id") || "");
  if (!fileId) {
    sendJson(response, 400, { error: "file_id richiesto" });
    return;
