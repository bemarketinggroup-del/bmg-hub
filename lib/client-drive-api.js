...
export default async function handler(request, response) {
  ...
  try {
    if (request.method === "POST" && action === "create-folder") {
      await createFolder(request, response, rootId);
      return;
    }
    if (request.method === "PATCH" && action === "rename") {
      await renameEntry(request, response, rootId);
      return;
    }
    if (request.method === "DELETE" && action === "trash") {
      await trashEntry(request, response, rootId);
      return;
    }
    ...
    } else if (!requestedFolderId || requestedFolderId === rootId) {
