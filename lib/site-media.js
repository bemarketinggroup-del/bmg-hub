...
async function ensureBucket() {
  const lookup = await fetch(`${SUPABASE_URL}/storage/v1/bucket`, { headers: storageHeaders() });
...
export async function handleSiteMedia(request, response) {
  ...
  try {
    ...
    response.end(JSON.stringify({ url: `${SUPABASE_URL}/storage/v1/object/public/${BUCKET}/${objectPath}` }));
  } catch (error) {
