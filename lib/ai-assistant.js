...
async function assistantContext(session, surfaceContext = {}, { includeHealth = false } = {}) {
  ...
  const [tasksResult, eventsResult, clientsResult, aliasesResult, notificationsResult, pedResult, stagingResult, teamRolesResult] = await Promise.all([
  ...
  const clientAppointments = canSeeClients
    ...
    : null;
  const clientHealth = canAnalyzeClients
    ? buildClientHealthSummaries({ clients: clientRows, pedItems: pedRows, stagingItems: stagingRows, tasks: taskRows, today, tasksAvailable: canAccessModule(session.profile, "tasks") }).slice(0, 8)
    : [];
  return {
