...
export function buildClientHealthSummaries({ clients = [], pedItems = [], stagingItems = [], tasks = [], today = "", tasksAvailable = true } = {}) {
  ...
  return (Array.isArray(clients) ? clients : [])
    ...
    .map((client) => {
      ...
      const copyScores = futureItems.map((item) => pedCopyScore(item.caption));
...
export async function handleClientHealth(request, response) {
  ...
  try {
    ...
    const [clientsResponse, pedResponse, stagingResponse, tasksResponse] = await Promise.all([
    ...
    const [clients, pedItems, stagingItems, tasks] = await Promise.all([
    ...
    const clientHealth = buildClientHealthSummaries({ clients, pedItems, stagingItems, tasks, today, tasksAvailable });
