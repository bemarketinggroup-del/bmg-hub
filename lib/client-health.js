  ...
  return (Array.isArray(clients) ? clients : [])
    ...
    .map((client) => {
      ...
      const allClientContent = [
        ...uniqueContent(pedItems.filter((item) => String(item.client_id || "") === clientId)),
        ...waitingItems
      ];
      ...
      const historicalCopiesByHash = new Map();
      for (const item of allClientContent) {
        const caption = String(item.caption || "").trim();
        if (!caption) continue;
        const hash = copyHash(caption);
        if (!historicalCopiesByHash.has(hash)) historicalCopiesByHash.set(hash, reviewsByCopy.get(`${clientId}:${hash}`) || null);
      }
      const historicalReviews = [...historicalCopiesByHash.values()].filter(Boolean);
      const historicalCopyScore = historicalReviews.length
        ? boundedScore(historicalReviews.reduce((sum, review) => sum + Number(review.overall_score || 0), 0) / historicalReviews.length)
        : 0;
      const historicalAnalysisScore = historicalCopiesByHash.size
        ? boundedScore(historicalReviews.length / historicalCopiesByHash.size * 100)
        : 0;
      const targetPosts = 15;
      const volumeScore = boundedScore(futureItems.length / targetPosts * 100);
      const formats = new Set(futureItems.map((item) => String(item.content_type || "post").toLowerCase()).filter(Boolean));
      const formatMixScore = !formats.size ? 0 : boundedScore(45 + Math.max(0, formats.size - 1) * 25);
      ...
      const scoreBreakdown = [
        { key: "coverage", label: "Copertura PED", score: coverageScore, weight: 15 },
        { key: "cadence", label: "Frequenza", score: cadenceScore, weight: 11 },
        { key: "volume", label: "Quantità contenuti", score: volumeScore, weight: 7 },
        { key: "format_mix", label: "Varietà formati", score: formatMixScore, weight: 4 },
        { key: "copy_quality", label: "Qualità copy futuri", score: copyScore, weight: 14 },
        { key: "copy_history", label: "Qualità copy storici", score: historicalCopyScore, weight: 6, available: historicalReviews.length > 0 },
        { key: "analysis", label: "Copy futuri analizzati", score: analysisScore, weight: 7 },
        { key: "analysis_history", label: "Archivio copy analizzato", score: historicalAnalysisScore, weight: 3, available: historicalCopiesByHash.size > 0 },
        ...
        { key: "appointments", label: "Appuntamenti", score: appointment.score, weight: 8, available: appointment.available }
      ];
      ...
      const summary = {
        ...
        historical_copy_score: historicalCopyScore,
        historical_analyzed_copies: historicalReviews.length,
        historical_total_copies: historicalCopiesByHash.size,
        historical_analysis_score: historicalAnalysisScore,
...
export async function handleClientHealth(request, response) {
  ...
  try {
    ...
    const [clientsResponse, aliasesResponse, pedResponse, stagingResponse, tasksResponse, reviewsResponse, profilesResponse, legacyProfilesResponse] = await Promise.all([
      ...
      supabaseFetch("/ped_items?select=id,client_id,scheduled_date,content_type,caption,content_group_id,publishing_status&order=scheduled_date.desc&limit=5000"),
