...
const TOKEN_CIPHER_CONTEXT = Buffer.from("bmg-hub:ped-share-token:v1");
...
function tokenEncryptionKey() {
  ...
  if (!secret) throw new Error("Secret condivisione PED non configurato");
  return createHash("sha256").update(`bmg-hub:ped-share:${secret}`).digest();
}

export function encryptPedShareToken(token, clientId) {
  if (!TOKEN_PATTERN.test(String(token || "")) || !String(clientId || "").trim()) {
    throw new Error("Token condivisione PED non valido");
  }
  const iv = randomBytes(12);
  const cipher = createCipheriv("aes-256-gcm", tokenEncryptionKey(), iv);
  cipher.setAAD(Buffer.concat([TOKEN_CIPHER_CONTEXT, Buffer.from(String(clientId))]));
...
export function decryptPedShareToken(value, clientId) {
  try {
    const [version, ivValue, tagValue, encryptedValue, extra] = String(value || "").split(".");
    if (version !== TOKEN_CIPHER_VERSION || !ivValue || !tagValue || !encryptedValue || extra) return "";
    const decipher = createDecipheriv("aes-256-gcm", tokenEncryptionKey(), Buffer.from(ivValue, "base64url"));
    decipher.setAAD(Buffer.concat([TOKEN_CIPHER_CONTEXT, Buffer.from(String(clientId))]));
    decipher.setAuthTag(Buffer.from(tagValue, "base64url"));
    const token = Buffer.concat([decipher.update(Buffer.from(encryptedValue, "base64url")), decipher.final()]).toString("utf8");
...
async function latestShare(clientId) {
  const result = await supabaseFetch(`/ped_share_links?select=id,client_id,token_ciphertext,share_month,is_active,expires_at,last_accessed_at,created_at,updated_at&client_id=eq.${encodeURIComponent(clientId)}&order=created_at.desc&limit=1`);
  if (!result.ok) return null;
  const rows = await result.json();
  return rows[0] || null;
}

async function activeShareSummary(clientId) {
  const result = await supabaseFetch(`/ped_share_links?select=id,client_id,token_ciphertext,share_month,is_active,expires_at,last_accessed_at,created_at,updated_at&client_id=eq.${encodeURIComponent(clientId)}&is_active=eq.true&order=created_at.desc`);
  if (!result.ok) return { latestActive: null, activeCount: 0, hasLegacyActive: false };
  const rows = await result.json();
  const activeRows = (rows || []).filter((row) => !row.expires_at || Date.parse(row.expires_at) > Date.now());
  return {
    latestActive: activeRows[0] || null,
    activeCount: activeRows.length,
    hasLegacyActive: activeRows.some((row) => !String(row.token_ciphertext || "").trim())
  };
}

function shareUrlFromToken(request, token, month = "") {
  if (!TOKEN_PATTERN.test(String(token || ""))) return "";
  const shareUrl = new URL(`${publicOrigin(request)}/ped-share`);
  if (/^\d{4}-\d{2}$/.test(String(month || ""))) shareUrl.searchParams.set("month", String(month));
  shareUrl.hash = token;
  return shareUrl.toString();
}

function shareStatus(row, request) {
  if (!row) return { active: false };
  const expired = Boolean(row.expires_at && Date.parse(row.expires_at) <= Date.now());
  const active = Boolean(row.is_active && !expired);
  const token = active ? decryptPedShareToken(row.token_ciphertext, row.client_id) : "";
  return {
    active,
    expired,
    ...
    share_url: token ? shareUrlFromToken(request, token, row.share_month) : "",
    recoverable: Boolean(token)
  };
}
...
async function revokeRecoverableClientShares(clientId) {
  return supabaseFetch(`/ped_share_links?client_id=eq.${encodeURIComponent(clientId)}&is_active=eq.true&token_ciphertext=not.is.null`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({ is_active: false, updated_at: new Date().toISOString() })
  });
...
export async function handlePedShareAdmin(request, response) {
  ...
  const session = await requireUser(request, response, { headers, roles: ["admin", "staff"], module: "ped" });
  if (!session) return;
  ...
  if (request.method === "GET") {
    ...
    const [latest, summary] = await Promise.all([
      latestShare(queryClientId),
      activeShareSummary(queryClientId)
    ]);
    sendJson(response, 200, {
      client: { id: client.id, name: client.name },
      ...shareStatus(summary.latestActive || latest, request),
      active_count: summary.activeCount,
      has_legacy_active: summary.hasLegacyActive
    }, headers);
    return;
  }
  ...
  if (request.method === "POST") {
    ...
    const shareMonth = String(body.share_month || "").trim();
    if (!clientId || !ALLOWED_EXPIRY_DAYS.has(expiryDays) || (shareMonth && !/^\d{4}-\d{2}$/.test(shareMonth))) {
    ...
    const activeSummary = await activeShareSummary(clientId);
    const currentStatus = shareStatus(activeSummary.latestActive, request);
    const preserveExisting = Boolean(body.preserve_existing && currentStatus.active && !currentStatus.recoverable);
    const revoked = preserveExisting
      ? await revokeRecoverableClientShares(clientId)
      : await revokeClientShares(clientId);
    if (!revoked.ok) {
      sendJson(response, revoked.status, { error: "Non riesco a preparare il nuovo link" }, headers);
      return;
    }
    ...
    const result = await supabaseFetch("/ped_share_links", {
      ...
      body: JSON.stringify({
        ...
        token_ciphertext: encryptPedShareToken(token, clientId),
        share_month: shareMonth || null,
        is_active: true,
    ...
    sendJson(response, 201, {
      ...
      active: true,
      expires_at: expiresAt,
      share_url: shareUrlFromToken(request, token, shareMonth),
      recoverable: true,
      active_count: preserveExisting ? Math.max(1, activeSummary.activeCount) + 1 : 1,
      has_legacy_active: preserveExisting,
      preserved_existing: preserveExisting
    }, headers);
    return;
  }
...
function publicMediaUrl(clientId, rootId, fileId, action, media = null) {
  const mediaToken = signDriveMediaToken({ clientId, rootId, fileId, action, ttlSeconds: 60 * 60, media });
...
function publicPedFile(item, rootId) {
  ...
  return {
    ...
    content_url: publicMediaUrl(item.client_id, rootId, item.drive_file_id, "content", signedMedia)
  };
}
