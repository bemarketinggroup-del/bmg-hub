...
function publicPedItem(item, rootId) {
  return {
    id: item.id,
...
export async function handlePublicPed(request, response) {
  ...
  const itemsResult = await supabaseFetch(`/ped_items?select=id,client_id,scheduled_date,content_type,drive_file_id,drive_file_name,drive_mime_type,drive_has_thumbnail,caption&client_id=eq.${encodeURIComponent(client.id)}&scheduled_date=gte.${range.start}&scheduled_date=lt.${range.end}&order=scheduled_date.asc,position.asc,created_at.asc`);
  if (!itemsResult.ok) {
  ...
  sendJson(response, 200, {
    ...
    items: items.map((item) => publicPedItem(item, rootId))
  }, headers);
}
