...
import { staffProfileEmails, staffProfileHasEmail } from "./staff-email-identities.js";
import { isGraphicDesigner } from "./professional-roles.js";
import { isCompletedTaskStatus } from "./task-completion-retention.js";

export { isCompletedTaskStatus } from "./task-completion-retention.js";
...
export async function personalAreaPayload(profile) {
  ...
  const assignedTasks = allTasks.filter((task) => taskAssignedToProfile(task, profile));
  const activeTasks = activePersonalTasks(assignedTasks);
  const personalEvents = events.filter((event) => eventIncludesProfile(event, profile));
  await createMissingNotifications(profile, activeTasks, personalEvents);
  const notifications = filterActiveNotifications(await loadUnreadNotifications(profile.id), activeTasks, profile);
  return { profile: publicProfile(profile), team, tasks: activeTasks, events: personalEvents, notifications };
}

export function activePersonalTasks(tasks) {
  return (Array.isArray(tasks) ? tasks : []).filter((task) => !isCompletedTaskStatus(task?.status));
}

export function filterActiveNotifications(notifications, activeTasks, profile = null) {
  const activeTaskIds = new Set((Array.isArray(activeTasks) ? activeTasks : [])
    .map((task) => String(task?.clickup_task_id || task?.id || "").trim())
    .filter(Boolean));
  return (Array.isArray(notifications) ? notifications : []).filter((notification) => {
    if (notification?.source_type === "graphic_review" && !isGraphicDesigner(profile)) return false;
    if (notification?.source_type !== "task") return true;
    return activeTaskIds.has(String(notification.source_id || "").trim());
  });
}
...
export function eventIncludesProfile(event, profile) {
  return Array.isArray(event?.attendees)
    && event.attendees.some((attendee) => staffProfileHasEmail(profile, attendee?.email, "calendar"));
}

async function loadActiveTeam() {
  const response = await supabaseFetch(
    "/staff_profiles?select=id,full_name,email,email_aliases,clickup_user_id,active&active=eq.true&order=full_name.asc"
  );
...
async function loadTasks() {
  const response = await supabaseFetch(
    "/clickup_tasks?select=clickup_task_id,name,description,status,priority,due_date_ms,assignees,tags,client_tag,clickup_url,payload,updated_at&order=updated_at.desc&limit=1000"
  );
...
function publicProfile(profile) {
  return { id: profile.id, email: profile.email, email_aliases: profile.email_aliases || [], full_name: profile.full_name, role: profile.role };
}
