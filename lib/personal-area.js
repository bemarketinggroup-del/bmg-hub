...
} from "./task-completion-retention.js";
...
export async function personalAreaPayload(profile) {
  ...
  const activeTasks = assignedTasks.filter((task) => !isCompletedTaskStatus(task.status));
  const tasks = assignedTasks
    .filter((task) => isTaskVisibleDuringCompletionRetention(task))
    .map((task) => ({ ...task, completed_at: taskCompletionTimestamp(task) }));
