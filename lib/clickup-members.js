...
async function clickupJson(path, options = {}) {
...
function clickUpErrorMessage(result, fallback) {
  const message = result?.data?.err
    || result?.data?.error
    || result?.data?.message
    || fallback;
  if ([401, 403].includes(Number(result?.status))) {
    return `${message}. L'invito utenti tramite API ClickUp richiede un workspace Enterprise e un token amministratore.`;
  }
  return message;
}

export async function ensureClickUpWorkspaceMember(email) {
  const normalizedEmail = String(email || "").trim().toLowerCase();
  const workspaceId = process.env.CLICKUP_WORKSPACE_ID || "90152036988";
  if (!normalizedEmail || !process.env.CLICKUP_API_TOKEN || !workspaceId) {
    return { ok: false, status: 500, error: "Integrazione ClickUp non configurata" };
  }

  const existingSource = await fetchClickUpMembers();
  const existing = existingSource.members.find((member) => member.email === normalizedEmail);
  if (existing) return { ok: true, member: existing, invited: false, pending: false };

  const invite = await clickupJson(`/team/${encodeURIComponent(workspaceId)}/user`, {
    method: "POST",
    body: JSON.stringify({ email: normalizedEmail, admin: false })
  });
  if (!invite.ok) {
    return {
      ok: false,
      status: invite.status,
      error: clickUpErrorMessage(invite, "Invito al workspace ClickUp non riuscito")
    };
  }

  const responseMember = normalizeClickUpMember(invite.data?.user || invite.data?.member || invite.data);
  if (responseMember?.email === normalizedEmail) {
    return { ok: true, member: responseMember, invited: true, pending: false };
  }

  const refreshedSource = await fetchClickUpMembers();
  const refreshed = refreshedSource.members.find((member) => member.email === normalizedEmail) || null;
  return {
    ok: true,
    member: refreshed,
    invited: true,
