...
const CALENDAR_REQUEST_TIMEOUT_MS = 10000;
...
export async function handleGoogleCalendar(request, response) {
  ...
  try {
    if (request.method === "GET") {
      ...
      if (url.searchParams.get("appointment_insights") === "1") {
        appointmentOverview = await googleCalendarAppointmentOverview({ events, timeMin, timeMax, clientTerms });
      }
      sendJson(response, 200, calendarPayload(events, appointmentOverview), headers);
      return;
    }
...
export function calendarPayload(events, appointmentOverview = null) {
  const payload = {
    calendar: {
      id: calendarId(),
  ...
  if (appointmentOverview) payload.appointment_overview = appointmentOverview;
  return payload;
}
...
export async function googleCalendarHealth() {
  ...
  try {
    ...
    await calendarRequest(`/events?${params}`);
    return {
      status: "online",
      checked_at: checkedAt,
      latency_ms: Date.now() - startedAt,
      auth_source: "dedicated_oauth",
      code: ""
    };
  } catch (error) {
    return {
...
async function googleCalendarAppointmentOverview({ events = [], timeMin = "", timeMax = "", clientTerms = [] } = {}) {
  const now = new Date();
  const horizon = new Date(now.getTime() + CLIENT_APPOINTMENT_WINDOW_DAYS * 24 * 60 * 60 * 1000);
  try {
    const requestedStart = new Date(timeMin);
    const requestedEnd = new Date(timeMax);
    const rangeCoversInsights = Number.isFinite(requestedStart.getTime())
      && Number.isFinite(requestedEnd.getTime())
      && requestedStart <= now
      && requestedEnd >= horizon;
    const [clientsResult, aliasesResult, insightEvents] = await Promise.all([
      supabaseFetch("/clients?select=id,name,status&status=neq.archiviato&status=neq.archived&order=name.asc"),
      supabaseFetch("/client_aliases?select=client_id,alias&order=alias.asc"),
      rangeCoversInsights ? Promise.resolve(events) : listCalendarEvents(now.toISOString(), horizon.toISOString(), clientTerms)
    ]);
    if (!clientsResult.ok || !aliasesResult.ok) return null;
    const [clients, aliases] = await Promise.all([clientsResult.json(), aliasesResult.json()]);
    return buildClientAppointmentOverview({
      clients,
      aliases,
      events: insightEvents,
      now,
      windowDays: CLIENT_APPOINTMENT_WINDOW_DAYS
    });
  } catch (error) {
    console.error("Google Calendar appointment overview failed", {
      name: error?.name,
  ...
  for (let attempt = 0; attempt <= CALENDAR_RETRY_DELAYS_MS.length; attempt += 1) {
    try {
      ...
      const response = await fetch(`${GOOGLE_CALENDAR_API}/calendars/${encodeURIComponent(calendarId())}${path}`, {
        ...options,
        signal: options.signal || AbortSignal.timeout(CALENDAR_REQUEST_TIMEOUT_MS),
        headers: {
      ...
      error.authSource = cachedToken?.source || "dedicated_oauth";
      lastError = error;
      ...
      if (!error.status) {
        error.status = 503;
        error.reason = error.name === "TimeoutError" ? "timeout" : "networkError";
      }
      lastError = error;
      if (!isRetryableGoogleError(error)) throw error;
    }

    if (attempt >= CALENDAR_RETRY_DELAYS_MS.length) throw lastError;
...
async function googleCalendarAccessToken() {
  ...
  if (clientId && clientSecret && refreshToken) {
    return googleCalendarOAuthToken({ clientId, clientSecret, refreshToken });
  }

  const error = new Error("OAuth dedicato Google Calendar non configurato");
  error.status = 503;
  error.reason = "calendar_oauth_not_configured";
  error.authSource = "dedicated_oauth";
  throw error;
}
...
function publicGoogleError(error) {
  ...
  if (error.reason === "calendar_oauth_not_configured") {
    return "Google Calendar richiede il collegamento OAuth dedicato di beviralagency@gmail.com.";
  }
...
function googleErrorCode(error) {
  ...
  if (error.reason === "calendar_oauth_not_configured") return "calendar_oauth_not_configured";
...
function isRetryableGoogleError(error) {
  if ([
    ...
    "accessNotConfigured",
    "SERVICE_DISABLED"
  ].includes(clean(error?.reason))) return false;
  return [408, 429, 500, 502, 503, 504].includes(Number(error?.status))
    || ["rateLimitExceeded", "userRateLimitExceeded", "backendError", "internalError", "networkError", "timeout"].includes(clean(error?.reason));
}
