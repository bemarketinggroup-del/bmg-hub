import { jsonHeaders, readJson, requireUser } from "../api/_auth.js";
...
export async function handleGoogleCalendar(request, response) {
  ...
  try {
    ...
    if (request.method === "POST") {
      ...
      const payload = buildGoogleEvent(body);
      const created = await calendarRequest("/events", {
        method: "POST",
        body: JSON.stringify(payload)
      });
      sendJson(response, 201, { event: normalizeGoogleEvent(created) }, headers);
      return;
    }
    ...
    if (request.method === "PATCH") {
      ...
      sendJson(response, 200, { event: normalizeGoogleEvent(updated) }, headers);
      return;
    }
...
export function normalizeGoogleEvent(event = {}) {
  ...
  const eventCategory = classifyGoogleCalendarEvent(event);
  return {
    id: String(event.id || ""),
...
export function buildGoogleEvent(body = {}) {
...
export function classifyGoogleCalendarEvent(event = {}) {
  ...
  if (/\b(smart working|smartworking|lavoro da remoto|remote working)\b/.test(text)) return "smart_working";
  if (/\b(appuntamento|appuntamenti|meeting|call|riunione|incontro)\b/.test(text)) return "client_appointment";
...
export async function listCalendarEvents(timeMin, timeMax) {
  ...
  return Array.isArray(data.items) ? data.items.map(normalizeGoogleEvent) : [];
}
