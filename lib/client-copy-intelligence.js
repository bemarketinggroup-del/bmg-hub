...
function authorizedCopyReviewCron(request) {
  const secret = clean(process.env.CRON_SECRET, 512);
  if (!secret) return false;
  return secureTextEqual(requestHeader(request, "authorization"), `Bearer ${secret}`);
}
...
async function analyzeCopy(session, body) {
  ...
  const reviewPayload = {
    ...
    reviewed_by: body.automated === true ? null : session.profile.id,
    model: OPENAI_COPY_REVIEW_MODEL,
  ...
  await auditReview(session, "suggestion", { ...billing, client_id: clientId, overall_score: combined.overallScore, profile_version: profile.profile_version, automated: body.automated === true });
...
async function copyReviewAutomationSession() {
  const profilesResult = await supabaseFetch("/staff_profiles?select=id,user_id,role,active,module_permissions&active=eq.true&user_id=not.is.null&limit=200");
  if (!profilesResult.ok) throw new Error("Profilo di servizio non disponibile");
  const profiles = await profilesResult.json().catch(() => []);
  const profile = profiles.find((item) => item.role === "admin") || profiles.find((item) => canAccessModule(item, "ped"));
  if (!profile?.id || !profile?.user_id) throw new Error("Nessun profilo attivo abilitato al PED");
  return { user: { id: profile.user_id }, profile, sessionId: "copy-review-cron" };
}

export async function runCopyReviewWorker({ limit = 3, owner = `cron-${randomUUID()}` } = {}) {
  const batchLimit = Math.max(1, Math.min(5, Number(limit) || 3));
  const queue = await pendingCopyReviewBatch(batchLimit, owner);
  if (queue.busy || queue.paused || !queue.candidates.length) {
    return {
      ok: true,
      busy: Boolean(queue.busy),
      paused: Boolean(queue.paused),
      pending_total: Number(queue.pending_total || 0),
      processed: 0,
      failed: 0,
      budget: queue.budget
    };
  }
  const session = await copyReviewAutomationSession();
  let processed = 0;
  let failed = 0;
  let stopped = false;
  let budget = queue.budget;
  for (const candidate of queue.candidates) {
    const result = await analyzeCopy(session, { ...candidate, automated: true });
    budget = result.body?.budget || budget;
    if (result.status >= 200 && result.status < 300) processed += 1;
    else failed += 1;
  ...
  return {
    ok: true,
    busy: false,
    paused: stopped && Boolean(budget?.blocked),
    pending_total: Number(queue.pending_total || 0),
    processed,
    failed,
    remaining_estimate: Math.max(0, Number(queue.pending_total || 0) - processed),
    has_more: Boolean(queue.has_more),
    budget
  };
}

export async function handleAiCopyReviewWorker(request, response) {
  if (request.method !== "GET") return json(response, 405, { error: "Method not allowed" });
  if (!authorizedCopyReviewCron(request)) return json(response, 401, { error: "Unauthorized" });
  try {
    const result = await runCopyReviewWorker({
      limit: process.env.COPY_REVIEW_CRON_BATCH_SIZE || 3,
      owner: `cron-${randomUUID()}`
    });
    return json(response, 200, result);
  } catch (error) {
    return json(response, 500, { error: clean(error?.message, 240) || "Worker analisi copy non disponibile" });
  }
}
