...
const COPY_REVIEW_POLICY_VERSION = 2;
...
async function verifyClientClaim(client, parsed) {
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            role: "system",
            content: "Verifica un solo fatto relativo a un cliente. Cerca prima nel sito ufficiale del cliente, poi in fonti affidabili. confirmed significa che una fonte sostiene il fatto; contradicted richiede una fonte che lo smentisca chiaramente; se non trovi prove sufficienti usa not_found. Rispondi in italiano, in modo prudente e in una sola frase."
          },
    ...
    return {
      status: ["confirmed", "not_found", "contradicted"].includes(result.status) ? result.status : "not_found",
      claim,
      note: clean(result.note, 240),
...
export function applyFactCheckEvidence(dimensions = {}, factCheck = null, warnings = []) {
  ...
  if (factCheck.status === "confirmed") {
    adjusted.relevance = Math.max(adjusted.relevance, 60);
    adjusted.brand_fit = Math.max(adjusted.brand_fit, 55);
    adjusted.factual_consistency = Math.max(adjusted.factual_consistency, 80);
...
async function analyzeCopy(session, body) {
      ...
      input: [
        {
          ...
          content: [
            ...
            "Chiedi fact_check_needed solo quando il copy contiene un'affermazione concreta e importante che non e verificabile nel profilo o nello storico. Indica una sola affermazione e una ricerca breve; non chiedere verifiche per opinioni, tono promozionale o formule creative.",
  ...
  const evidence = applyFactCheckEvidence(parsed.dimensions, factCheck, parsed.fact_check_needed ? [] : parsed.context_warnings);
  ...
  const reviewPayload = {
    ...
    summary: clean(factCheck?.status === "confirmed" ? factCheck.note || parsed.summary : parsed.summary, 240),
    dimensions: {
      ...combined.dimensions,
    ...
    improvements: factCheck?.status === "confirmed" ? [] : stringList(parsed.improvements, 1),
