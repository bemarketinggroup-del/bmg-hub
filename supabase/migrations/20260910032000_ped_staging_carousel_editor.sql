create or replace function public.sync_ped_staging_carousel_members(
  p_group_id uuid,
  p_member_ids uuid[]
)
returns void
...
begin
  select count(*)
    into current_count
    from public.ped_staging_items
   where content_group_id = p_group_id
     and content_type = 'carousel';

  if current_count < 2 then
    raise exception 'Carosello momentaneo non trovato';
  end if;
  ...
  delete from public.ped_staging_items
   where content_group_id = p_group_id
     and content_type = 'carousel'
...
revoke all on function public.sync_ped_staging_carousel_members(uuid, uuid[]) from public;
grant execute on function public.sync_ped_staging_carousel_members(uuid, uuid[]) to service_role;
