...
let pedGalleryShareState = { loadId: 0, groupId: "", files: [], nextIndex: 0 };
let pedLoadingKey = "";
...
function syncPedStagingEditorMedia(item) {
  ...
  const canAppend = pedContentType(item.content_type) !== "story";
...
function pedAgendaItemMarkup(item) {
  ...
  const singleDownloadLabel = isIosDownloadDevice() ? "Foto" : "Scarica";
  ...
  const singleDownloadTitle = isIosDownloadDevice()
    ? `Salva ${primary.drive_file_name || title} nella galleria Foto`
    : `Scarica ${primary.drive_file_name || title}`;
...
function openPedCarouselDownloadList(item, files) {
  ...
  document.getElementById("pedDownloadTitle").textContent = isSingle ? "Scarica il contenuto" : "Scarica il multipost";
  subtitle.textContent = isSingle ? String(files[0]?.drive_file_name || "1 file") : `${files.length} file · da 01 a ${String(files.length).padStart(2, "0")}`;
  intro.textContent = isSingle
    ? "Questo formato non può essere inviato direttamente a Foto. Tocca il file per scaricarlo."
    : "Tocca ogni file nell'ordine indicato. Il browser lo invierà ai Download senza creare uno ZIP.";
  message.textContent = isSingle ? "Download alternativo disponibile." : "Inizia dal file 01 e prosegui fino all'ultimo.";
...
function pedGalleryItemStatus(index, status, label) {
  ...
  row.classList.toggle("is-saved", status === "saved");
...
function closePedGalleryModal() {
  pedGalleryShareState = { loadId: pedGalleryShareState.loadId + 1, groupId: "", files: [], nextIndex: 0 };
  document.getElementById("pedDownloadModal").close();
}

function updatePedGalleryNextItem() {
  const { files, nextIndex } = pedGalleryShareState;
  const button = document.getElementById("pedGalleryShareButton");
  document.querySelectorAll("[data-ped-gallery-item]").forEach((row) => {
    row.classList.toggle("is-next", Number(row.dataset.pedGalleryItem) === nextIndex && nextIndex < files.length);
  });
  if (nextIndex >= files.length) {
    button.disabled = true;
    button.textContent = "Sequenza completata";
    return;
  }
  button.disabled = false;
  button.textContent = files.length === 1
    ? "Salva in Foto"
    : `Salva ${String(nextIndex + 1).padStart(2, "0")} in Foto`;
}
...
async function preparePedCarouselForGallery(item, files, loadId) {
  ...
  try {
    ...
    if (
      ...
      || preparedFiles.some((file) => !navigator.canShare({ files: [file] }))
    ) {
      throw new Error("Questo iPhone non consente di inviare uno o più contenuti all'app Foto");
    }
    pedGalleryShareState.files = preparedFiles;
    pedGalleryShareState.nextIndex = 0;
    updatePedGalleryNextItem();
    message.textContent = orderedMediaCount === preparedFiles.length
      ? preparedFiles.length === 1
        ? `Tocca “Salva in Foto” e scegli il comando di salvataggio nella galleria dell'iPhone.`
        : `Tocca “Salva 01 in Foto” e scegli “Salva in Foto”. Al ritorno passerai automaticamente al contenuto 02.`
      : `${orderedMediaCount} contenuti hanno metadata ordinati. Salvali comunque uno alla volta per non farli raggruppare per formato.`;
  } catch (error) {
    if (pedGalleryShareState.loadId !== loadId) return;
...
function openPedCarouselGallery(item, files) {
  ...
  pedGalleryShareState = { loadId, groupId: String(item.id || ""), files: [], nextIndex: 0 };
  ...
  list.innerHTML = files.map((file, index) => {
    ...
    return `<div class="ped-download-item" data-ped-gallery-item="${index}">
      <span class="ped-download-number">${String(index + 1).padStart(2, "0")}</span>
      <span class="ped-download-file"><strong>${escapeHtml(file.drive_file_name || filename)}</strong><small>${isSingle ? "Salvataggio nella galleria Foto" : `Passaggio ${String(index + 1).padStart(2, "0")} · salvataggio singolo`}</small></span>
...
function sharePreparedPedGallery() {
  const { files, nextIndex, loadId } = pedGalleryShareState;
  const message = document.getElementById("pedDownloadMessage");
  const button = document.getElementById("pedGalleryShareButton");
  const file = files[nextIndex];
  if (!file || typeof navigator.share !== "function") return;
  const position = String(nextIndex + 1).padStart(2, "0");
  button.disabled = true;
  pedGalleryItemStatus(nextIndex, "saving", "Apro");
  message.textContent = `Contenuto ${position}: nel pannello iPhone scegli “Salva in Foto”.`;
  let shareResult;
  try {
    shareResult = navigator.share({ files: [file] });
  } catch (error) {
    pedGalleryItemStatus(nextIndex, "ready", "Riprova");
    updatePedGalleryNextItem();
  ...
  Promise.resolve(shareResult).then(() => {
    if (pedGalleryShareState.loadId !== loadId || pedGalleryShareState.nextIndex !== nextIndex) return;
    pedGalleryItemStatus(nextIndex, "saved", "Salvato");
    pedGalleryShareState.nextIndex += 1;
    updatePedGalleryNextItem();
    if (pedGalleryShareState.nextIndex >= files.length) {
      message.textContent = files.length === 1
        ? "Contenuto consegnato al pannello iPhone per il salvataggio nella galleria Foto."
        : `${files.length} contenuti completati. Foto e video sono stati consegnati nella sequenza del PED.`;
      return;
    }
    const nextPosition = String(pedGalleryShareState.nextIndex + 1).padStart(2, "0");
    message.textContent = `Contenuto ${position} completato. Ora tocca “Salva ${nextPosition} in Foto”.`;
  }).catch((error) => {
    if (pedGalleryShareState.loadId !== loadId) return;
    pedGalleryItemStatus(nextIndex, "ready", "Riprova");
    updatePedGalleryNextItem();
    if (error?.name === "AbortError") {
      message.textContent = `Salvataggio ${position} annullato. Tocca di nuovo per riprovare senza saltare la posizione.`;
      return;
    }
    message.textContent = error.message || "Non riesco ad aprire il pannello di condivisione iPhone.";
  });
...
async function downloadPedSingleMedia(itemId, button) {
  const item = pedStateItem(itemId);
  const file = item ? pedItemFiles(item)[0] : null;
  if (!item || !file || (!file.content_url && !file.download_url)) {
    alert("Il contenuto non è disponibile per il download");
    return;
  }

  if (isIosDownloadDevice()) {
    openPedCarouselGallery(item, [file]);
    return;
  }

  await downloadDriveResource(
    file.download_url || file.content_url,
    file.drive_file_name || pedItemTitle(item),
    button
...
async function openPedDrivePicker(date = "", { appendItem = null, stagingAppendItem = null, staging = false } = {}) {
  ...
  const appendMode = Boolean(appendTarget && pedContentType(appendTarget.content_type) !== "story");
  ...
  document.getElementById("pedPickerTitle").textContent = appendMode
    ? appendTarget.is_group ? "Aggiungi contenuti al carosello" : "Trasforma in carosello"
    : stagingMode
  ...
  document.getElementById("pedPickerSubtitle").textContent = appendMode
    ? appendTarget.is_group
      ? `${client.name} · il carosello contiene gia ${existingCount} ${existingCount === 1 ? "contenuto" : "contenuti"}`
      : `${client.name} · scegli almeno un altro contenuto: diventera automaticamente un carosello`
    : stagingMode
...
function isPedDriveFileStaged(file) {
  return !file?.is_folder && pedStagingFileIds.has(String(file.id));
}

function setPedPickerMessage(text = "", tone = "") {
...
function renderPedPicker() {
  ...
  grid.innerHTML = libraryCards + sortedVisibleFiles.map((file, index) => {
    ...
    const staged = isPedDriveFileStaged(file);
    ...
    const mediaContent = `${hasPreview
      ...
      : driveFileIcon(file)}${used ? `<span class="ped-picker-used-badge${staged ? " is-staged" : ""}">${staged ? "Gia in attesa" : "Gia nel PED"}</span>` : ""}${isCarouselSelection && selected ? `<strong class="ped-picker-order-badge" title="${selectionOrder === 1 ? "Copertina del carosello nel feed Instagram" : `Posizione ${selectionOrder} nel carosello`}">${selectionOrder}</strong>` : ""}`;
    if (file.is_folder) {
    ...
    const insertLabel = isCarouselSelection && selected
      ? "Rimuovi dal carosello"
      : staged
        ? "Aggiungi di nuovo"
        : "Inserisci nel PED";
...
function togglePedCarouselFile(fileId) {
  ...
  if (index >= 0) pedPickerState.selectedFiles.splice(index, 1);
    ...
    if (Number(pedPickerState.existingCount || 0) + pedPickerState.selectedFiles.length >= 20) {
      setPedPickerMessage("Il carosello puo contenere al massimo 20 contenuti.", "error");
      return;
    }
...
async function attachPedDriveFiles(fileIds, { caption = pedPickerState.caption, captionHtml = null, messageTargetId = "pedPickerMessage" } = {}) {
  ...
  if (appendMode && !fileIds.length) {
    setPedPickerMessage("Seleziona almeno un nuovo contenuto da aggiungere.", "error");
    return;
  }
...
function openPedCarouselPreviewWithOptions(item, { readOnly = false, staging = false } = {}) {
  ...
  function renderCards() {
    ...
    files.forEach((file, index) => {
      ...
      moveLeft.className = "ped-carousel-editor-move p-button p-component p-button-text p-button-rounded p-button-icon-only";
      ...
      moveLeft.innerHTML = `<svg class="lc p-button-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m15 18-6-6 6-6"/></svg>`;
      moveLeft.addEventListener("click", () => moveFile(id, memberId(files[index - 1]), false));
      const moveRight = document.createElement("button");
      moveRight.type = "button";
      moveRight.className = "ped-carousel-editor-move p-button p-component p-button-text p-button-rounded p-button-icon-only";
      ...
      moveRight.innerHTML = `<svg class="lc p-button-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m9 18 6-6-6-6"/></svg>`;
      moveRight.addEventListener("click", () => moveFile(id, memberId(files[index + 1]), true));
      const remove = document.createElement("button");
      remove.type = "button";
      remove.className = "ped-carousel-editor-remove p-button p-component p-button-outlined p-button-danger";
      ...
      remove.innerHTML = `<svg class="lc p-button-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 15H6L5 6"/><path d="M10 11v5M14 11v5"/></svg><span class="p-button-label">Rimuovi</span>`;
...
function renderSmartMonth(data) {
  ...
  target.innerHTML = visibleWeeks.map((weekDates) => {
    ...
    const plan = (data.plans || []).find((item) => item.week_start_date === weekDates[0]);
    const actionable = weekDates[0] >= currentWeek;
    const planStatus = plan?.status === "approved" ? "Pubblicata" : plan?.status === "draft" ? "Bozza pronta" : "Da generare";
    const weekActions = data.can_manage ? `<div class="smart-week-actions" aria-label="Azioni settimana ${escapeHtml(smartWeekDateLabel(weekDates[0]))}">
      <button class="ghost-button" type="button" data-smart-week-action="sync" data-smart-week-start="${escapeHtml(weekDates[0])}" title="Aggiorna da Google Calendar soltanto questa settimana">Sincronizza</button>
      <button class="ghost-button" type="button" data-smart-week-action="generate" data-smart-week-start="${escapeHtml(weekDates[0])}" ${actionable && !smartWorkingLoading ? "" : "disabled"} title="Crea la proposta iniziale soltanto per questa settimana">${plan ? "Rigenera proposta" : "Genera proposta"}</button>
      <button class="primary-button" type="button" data-smart-week-action="publish" data-smart-week-start="${escapeHtml(weekDates[0])}" ${actionable && plan?.status === "draft" && !smartWorkingLoading ? "" : "disabled"} title="Pubblica su Google Calendar soltanto questa settimana">${plan?.status === "approved" ? "Pubblicata" : "Pubblica"}</button>
    </div>` : "";
    return `<section class="smart-month-week${summary.future ? " is-future" : ""}${selectedWeek ? " is-selected" : ""}" data-smart-week="${escapeHtml(weekDates[0])}">
      <header class="smart-week-toolbar">
        <button class="smart-week-status${summary.complete ? " is-complete" : " is-incomplete"}" type="button" data-smart-week-select="${escapeHtml(weekDates[0])}" aria-pressed="${selectedWeek ? "true" : "false"}">
          <span class="smart-week-heading"><strong>${escapeHtml(summary.label)}</strong><i class="is-${escapeHtml(plan?.status || "empty")}">${escapeHtml(planStatus)}</i></span>
...
async function syncSmartWeek(weekStart, button) {
  ...
  try {
    const data = await smartWorkingAction("sync_week", { week_start: weekStart, force: true });
    smartWorkingLastSyncedAt = new Date().toISOString();
    renderSmartWorking();
    const invited = Number(data.result?.invited) || 0;
    const invitationErrors = Number(data.result?.invitation_errors) || 0;
    alert(`Settimana ${smartWeekDateLabel(weekStart)} sincronizzata. ${data.result?.cached || 0} eventi letti e ${data.result?.blocked || 0} impegni associati${invited ? `, ${invited} nuovi inviti inviati` : ""}${invitationErrors ? `, ${invitationErrors} inviti non riusciti` : ""}. Nessuno smart è stato pubblicato.`);
  } catch (error) {
    renderBackendStatus(error.message);
    alert(error.message || "Non riesco a sincronizzare questa settimana con Google Calendar.");
  } finally {
    renderSmartWorking();
  }
}
...
async function generateSmartWeek(weekStart, button) {
  ...
  try {
    ...
    alert(`Proposta della settimana ${smartWeekDateLabel(weekStart)} generata: ${data.result?.created || 0} assegnazioni${conflicts ? `, ${conflicts} conflitti da verificare` : ""}. Ora puoi spostare i nomi verdi e pubblicare quando è pronta.`);
  } catch (error) {
    renderBackendStatus(error.message);
...
function selectGraphicsDriveClient(client) {
  ...
  const isAlreadyOpen = clientDriveState.surface === "graphics"
    && String(clientDriveState.clientId) === String(client.id)
    && String(graphicsDriveClientId) === String(client.id);
  if (isAlreadyOpen) {
    closeGraphicsClientDrive();
    return;
  }
...
document.body.addEventListener("dragstart", (event) => {
  ...
  if (smartChip) {
    ...
    document.querySelectorAll(".smart-month-day[data-smart-date]").forEach((day) => {
      if (!day.classList.contains("is-weekend") && day.dataset.smartDate >= localDateKey(new Date()) && smartWeekStart(day.dataset.smartDate) === sourceWeek) day.classList.add("is-smart-drop-ready");
    });
    event.dataTransfer.effectAllowed = "move";
...
document.getElementById("smartView").addEventListener("click", (event) => {
  const weekAction = event.target.closest("[data-smart-week-action][data-smart-week-start]");
  if (weekAction) {
    event.stopPropagation();
    const weekStart = weekAction.dataset.smartWeekStart;
    selectedSmartDate = weekStart;
    scheduleWorkspaceContextSave();
    if (weekAction.dataset.smartWeekAction === "sync") void syncSmartWeek(weekStart, weekAction);
    else if (weekAction.dataset.smartWeekAction === "generate") void generateSmartWeek(weekStart, weekAction);
    else if (weekAction.dataset.smartWeekAction === "publish") void approveSmartWeek(weekStart, weekAction);
    return;
  }
