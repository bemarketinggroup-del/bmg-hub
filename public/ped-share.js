...
function shareTypeIconMarkup(value) {
  ...
  return `<span class="share-type-icon is-${type}" aria-label="${escapeHtml(PED_TYPES[type].label)}" title="${escapeHtml(PED_TYPES[type].label)}"><svg viewBox="0 0 24 24" aria-hidden="true">${icon}</svg></span>`;
}
...
function renderItem(item) {
  ...
  return `<button class="share-item type-${type}" data-share-item="${escapeHtml(item.id)}" type="button" title="Apri anteprima di ${escapeHtml(item.file_name)}">
    <span class="share-item-media">${media}${shareTypeIconMarkup(type)}${files.length > 1 ? `<i class="share-carousel-count">${files.length}</i>` : ""}</span>
