...
assert.match(assistant, /buildClientAffinities/, "l'assistente deve imparare i clienti dallo storico delle modifiche PED");
assert.match(assistant, /client_assignments/, "il contesto AI deve dichiarare le associazioni cliente-persona apprese");
assert.match(assistant, /assistantContext\(session, surfaceContext, \{ includeHealth: true \}\)/, "anche l'assistente contestuale deve poter analizzare la salute dei clienti pertinenti");
