...
assert.match(htmlSource, /team-chat-room-head[\s\S]*team-chat-live/, "lo stato live deve stare nella testata della conversazione senza sottrarre altezza alla chat");
assert.match(cssSource, /\.team-chat-workspace/);
assert.match(cssSource, /\.team-chat-layout\s*\{[\s\S]*flex:\s*1 1 0/);
assert.match(cssSource, /body\.chat-view-active\s*\{\s*overflow:\s*hidden/, "la pagina chat non deve avere scroll esterno");
assert.match(cssSource, /body\.chat-view-active \.main\s*\{[\s\S]*grid-template-rows:\s*auto minmax\(0, 1fr\)[\s\S]*overflow:\s*hidden/, "la chat deve occupare tutto lo spazio sotto la topbar");
assert.match(appSource, /document\.body\.classList\.toggle\("chat-view-active", view === "chat"\)/, "la modalità a viewport bloccato deve attivarsi solo nella chat");
