...
assert.match(styleSource, /\.ped-media-viewer-stage > \.media-load-progress \{[\s\S]*?position: static;/, "il caricamento foto deve restare sotto al media");
assert.match(styleSource, /\.ped-media-viewer-stage > \.media-load-progress progress \{[^}]*height: 3px;/, "il caricamento foto deve restare compatto");
...
assert.match(appSource, /const progress = stage\.querySelector\(":scope > \[data-media-progress\]"\);[\s\S]*?surface\.append\(progress\)/, "il buffering video deve essere spostato dentro la superficie video");
assert.match(styleSource, /\.media-viewer-video-surface > \.media-load-progress,[\s\S]*?position: absolute;[\s\S]*?right: 10px;[\s\S]*?bottom: 10px;[\s\S]*?width: min\(158px, calc\(100% - 20px\)\)/, "il buffering deve essere un indicatore piccolo in basso a destra");
assert.match(styleSource, /\.media-viewer-video-surface > \.media-load-progress progress,[\s\S]*?height: 2px;/, "la barra del buffering video deve essere molto sottile");
assert.match(styleSource, /\.media-viewer-video-surface > \.media-load-progress\.is-hidden,[\s\S]*?visibility: hidden;[\s\S]*?opacity: 0;/, "la scomparsa del buffering non deve cambiare il layout del player");
assert.match(appSource, /progress\?\.classList\.add\("is-hidden"\);[\s\S]*?showEmbeddedDriveVideo\(mediaRoot, fileId, name\)/, "il fallback Drive non deve mostrare due barre di caricamento");
assert.match(appSource, /const scheduledDays = \[\.\.\.grouped\.entries\(\)\]/, "l'agenda deve derivare i giorni dai contenuti programmati");
assert.match(appSource, /pedAgendaShowsPrevious\s*\? allItems\s*:\s*allItems\.filter/, "l'agenda deve mostrare i precedenti soltanto su richiesta");
assert.match(appSource, /previousLabel\.textContent = pedAgendaShowsPrevious \? "Nascondi precedenti" : `Carica precedenti \(\$\{previousItemsCount\}\)`/, "il comando deve caricare e poter nascondere i contenuti precedenti");
assert.match(appSource, /state\.pedAgendaItems/, "l'agenda deve usare i contenuti di tutti i mesi");
assert.match(appSource, /agenda_from: localDateKey\(new Date\(\)\)/, "il client deve richiedere tutti i contenuti da oggi");
assert.match(appSource, /include_history: "1"/, "il client deve richiedere lo storico completo del PED");
assert.match(pedSource, /includeHistory \? "" : `&scheduled_date=gte\.\$\{agendaFrom\}`/, "l'API deve poter includere lo storico senza il filtro data");
...
assert.match(htmlSource, /id="pedAgendaPrevious"/, "l'agenda deve mostrare il comando Carica precedenti");
...
assert.match(appSource, /class="ped-instagram-past-dot"/, "i post passati devono avere un indicatore dedicato");
assert.match(styleSource, /\.ped-instagram-past-dot,[\s\S]*?bottom: 7px;[\s\S]*?background: #d64138;/, "il pallino rosso dei post passati deve restare piccolo e in basso");
assert.match(appSource, /filter\(\(item\) => !pedInstagramItemIsPast\(item\)\)\.map/, "il riordino deve escludere i contenuti passati");
...
assert.match(styleSource, /\.ped-agenda-preview \{[\s\S]*?overflow: hidden;[\s\S]*?border: 0;[\s\S]*?background: var\(--surface-tint\);/, "la miniatura agenda non deve mostrare una cornice o un fondale nero");
...
assert.match(styleSource, /\.ped-content-thumb > img,[\s\S]*?\.ped-caption-day-thumb > img \{[\s\S]*?position: absolute;[\s\S]*?inset: 0;[\s\S]*?object-fit: cover !important;/, "tutte le miniature PED devono riempire il rettangolo senza fasce residue");
assert.match(appSource, /function pedTypeIconMarkup[\s\S]*?post:[\s\S]*?story:[\s\S]*?reel:[\s\S]*?carousel:/, "post, stories, reel e multipost devono avere simboli dedicati");
assert.match(appSource, /ped-staging-thumb">\$\{media\}\$\{pedTypeIconMarkup\(format\.type\)\}/, "i contenuti in attesa devono mostrare il simbolo editoriale");
assert.match(appSource, /ped-content-thumb">\$\{media\}\$\{pedTypeIconMarkup\(format\.type\)\}/, "il calendario deve mostrare il simbolo editoriale");
assert.match(appSource, /ped-agenda-preview[\s\S]{0,400}\$\{media\}\$\{pedTypeIconMarkup\(format\.type\)\}/, "l'agenda deve mostrare il simbolo editoriale");
assert.match(styleSource, /\.ped-media-type-badge \{[\s\S]*?position: absolute;[\s\S]*?z-index: 4;/, "il simbolo deve restare piccolo e sovrapposto nell'angolo");
assert.match(styleSource, /\.ped-agenda-preview\.p-button > img \{ position: absolute; z-index: 0; \}/, "PrimeNG non deve spostare la foto fuori dal livello full-bleed");
assert.match(styleSource, /\.ped-agenda-preview\.p-button > \.ped-media-type-badge \{ position: absolute; z-index: 4; \}/, "PrimeNG non deve spostare il simbolo editoriale");
assert.match(shareSource, /shareTypeIconMarkup\(type\)/, "anche il PED condiviso deve mostrare il simbolo editoriale");
assert.match(shareStyleSource, /\.share-item-media img \{[\s\S]*?position: absolute;[\s\S]*?inset: 0;[\s\S]*?object-fit: cover;/, "il PED condiviso deve usare miniature full-bleed");
...
assert.match(appSource, /\["pedCreateCaptionText", "pedCaptionText", "pedStagingText"\]\.forEach[\s\S]*?addEventListener\("paste", handlePedEditorPaste\)/, "la normalizzazione dell'incolla deve valere durante la creazione, sui contenuti programmati e in attesa");
...
assert.match(styleSource, /#pedCreateCaptionForm,\s*#pedCaptionForm,\s*#pedStagingEditorForm \{[\s\S]*?grid-template-rows: auto auto auto minmax\(100px, 1fr\) auto auto auto;[\s\S]*?overflow: hidden;/, "testata, controlli, valutazione e azioni devono restare nella finestra fissa anche durante la creazione");
...
assert.match(styleSource, /@media \(max-width: 680px\) \{[\s\S]*?\.ped-caption-modal,[\s\S]*?left: max\(8px, env\(safe-area-inset-left\)\);[\s\S]*?right: max\(8px, env\(safe-area-inset-right\)\);[\s\S]*?width: auto;/, "su smartphone il popup copy deve restare dentro entrambi i margini sicuri");
assert.match(styleSource, /\.ped-caption-modal \.ped-caption-actions \{[\s\S]*?display: grid;[\s\S]*?grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);/, "le azioni del popup copy devono andare su righe mobili senza allargare la finestra");
assert.match(styleSource, /#pedCaptionForm > \*[\s\S]*?min-width: 0; max-width: 100%;/, "ogni sezione del popup copy deve potersi restringere alla larghezza del telefono");
assert.match(styleSource, /@media \(max-height: 720px\) \{[\s\S]*?\.ped-caption-modal \.ped-copy-source,[\s\S]*?\.ped-caption-modal \.ped-copy-ai-row \{ display: none; \}/, "sui telefoni bassi i dettagli secondari non devono spingere fuori le azioni principali");
...
assert.match(htmlSource, /class="ped-picker-setup"[\s\S]*?1\. Scegli i contenuti/, "il selettore deve presentare chiaramente il primo passaggio dedicato ai media");
assert.match(htmlSource, /class="ped-picker-browser-bar"/, "percorso e comandi Drive devono condividere una barra compatta");
assert.doesNotMatch(htmlSource, /id="pedPickerCaption"/, "il selettore media non deve più comprimere il copy in un campo piccolo");
assert.match(htmlSource, /id="pedCreateCaptionModal"[\s\S]*?id="pedCreateCaptionText"[\s\S]*?id="pedCreateCaptionSaveButton"/, "dopo i media deve esistere una finestra ampia e dedicata al copy");
assert.match(appSource, /function openPedCreateCaptionStep\(\)[\s\S]*?pickerModal\.close\(\)[\s\S]*?modal\.showModal\(\)/, "la conferma dei media deve aprire il secondo passaggio senza salvare contenuti parziali");
assert.match(appSource, /if \(pedPickerFile\) \{\s*return togglePedCarouselFile/, "anche foto e video singoli devono essere selezionati prima di proseguire");
assert.match(appSource, /if \(pedCreateCarousel\) return openPedCreateCaptionStep\(\)/, "il comando fisso deve continuare verso il copy invece di salvare subito");
assert.match(appSource, /caption_html: format === "story" \? null : captionHtml/, "la creazione finale deve conservare anche la formattazione del copy");
