...
assert.match(styleSource, /@media \(max-width: 640px\) and \(hover: none\) and \(pointer: coarse\)[\s\S]*?\.sidebar \{[\s\S]*?position: fixed;[\s\S]*?transform: translateX\(-105%\);/, "solo su smartphone la sidebar deve diventare un drawer laterale");
...
assert.match(appSource, /matchMedia\("\(max-width: 640px\) and \(hover: none\) and \(pointer: coarse\)"\)/, "il burger deve attivarsi solo su dispositivi touch piccoli");
