...
assert.match(styleSource, /MOBILE EDGE-TO-EDGE[\s\S]*?@media \(max-width: 640px\)[\s\S]*?body \.app-shell > \.main \{[\s\S]*?padding-right: max\(4px, env\(safe-area-inset-right\)\);[\s\S]*?padding-left: max\(4px, env\(safe-area-inset-left\)\);/, "tutte le pagine smartphone devono usare quasi tutta la larghezza rispettando le safe area");
assert.match(styleSource, /MOBILE EDGE-TO-EDGE[\s\S]*?#calendarView \{[\s\S]*?margin-right: 0;[\s\S]*?margin-left: 0;[\s\S]*?\.smart-calendar-panel \{[\s\S]*?width: 100%;[\s\S]*?margin-right: 0;[\s\S]*?margin-left: 0;/, "Calendario e Smart Working non devono conservare compensazioni laterali del vecchio gutter mobile");
