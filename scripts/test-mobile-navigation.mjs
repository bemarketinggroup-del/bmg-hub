...
assert.match(styleSource, /MOBILE EDGE-TO-EDGE[\s\S]*?#contentView > \.cms-workspace,[\s\S]*?#pedView > \.ped-workspace,[\s\S]*?#clientHealthOverviewView > \.client-health-overview-workspace,[\s\S]*?#clientHealthView > \.client-health-workspace,[\s\S]*?#chatView > \.team-chat-workspace,[\s\S]*?#calendarView > \.google-calendar-workspace,[\s\S]*?#teamView > \.task-workspace,[\s\S]*?#usersView > \.users-workspace,[\s\S]*?#smartView \.smart-calendar-panel \{[\s\S]*?padding-right: 0;[\s\S]*?padding-left: 0;/, "tutti i workspace strutturali devono eliminare il padding laterale reintrodotto dal tema");
