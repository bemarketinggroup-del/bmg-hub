...
assert.match(smartAppSource, /proposal && data\.can_move_smart && smartWeekStart\(item\.date\) >= smartWeekStart/, "le proposte della settimana corrente e futura devono essere trascinabili");
...
assert.match(smartApiSource, /staffCanRefreshCalendar = \["sync_calendar", "sync_off_year", "sync_week"\]\.includes\(action\)/, "lo staff deve poter mantenere aggiornata la propria vista Calendar");
