...
assert.doesNotMatch(smartHtmlSource, /id="(?:syncCalendarButton|generateSmartWeekButton|approveSmartWeekButton)"/, "le azioni non devono piu appartenere alla toolbar mensile");
assert.match(smartHtmlSource, /class="smart-week-flow"[\s\S]*?sincronizza, genera la proposta, sposta i nomi verdi e pubblica/, "il flusso settimanale deve essere spiegato in modo semplice");
assert.match(smartAppSource, /Sola lettura · aggiornamento gestito dall'amministratore/, "lo staff deve vedere chiaramente che Calendar viene aggiornato dall'admin");
assert.match(smartAppSource, /<strong>Sola lettura:<\/strong> soltanto l'amministratore può modificare e pubblicare i turni/, "lo staff deve ricevere istruzioni coerenti con la vista non modificabile");
...
assert.match(smartAppSource, /data-smart-week-action="sync"[\s\S]*?data-smart-week-action="generate"[\s\S]*?data-smart-week-action="publish"/, "ogni settimana deve mostrare sincronizzazione, proposta e pubblicazione");
assert.match(smartAppSource, /smartWorkingAction\("sync_week", \{ week_start: weekStart, force: true \}\)/, "la sincronizzazione manuale deve riguardare soltanto la settimana scelta");
...
assert.match(smartAppSource, /proposal && data\.can_move_smart && smartWeekStart\(item\.date\) >= smartWeekStart/, "le proposte della settimana corrente e futura devono essere trascinabili dall'admin");
assert.match(smartAppSource, /day\.dataset\.smartDate >= localDateKey\(new Date\(\)\)/, "il trascinamento della settimana corrente non deve accettare giorni gia trascorsi");
...
assert.match(smartAppSource, /function refreshSmartWorkingInBackground[\s\S]*?currentProfile\?\.role !== "admin"[\s\S]*?smartWorkingAction\("sync_calendar"/, "la sincronizzazione automatica deve essere avviata soltanto dall'admin");
assert.match(smartAppSource, /function startSmartWorkingUpdates\(\)[\s\S]*?currentProfile\?\.role !== "admin"[\s\S]*?refreshSmartWorkingInBackground\(smartMonthKey\(\), \{ refresh: true \}\)/, "il timer admin deve aggiornare Calendar senza ricaricare la pagina");
...
assert.doesNotMatch(smartApiSource, /staffCanMoveSuggestion|staffCanRefreshCalendar/, "il server non deve mantenere eccezioni di scrittura per lo staff");
assert.match(smartApiSource, /const action = clean\(body\.action\);\s*if \(session\.profile\.role !== "admin"\) \{\s*sendJson\(response, 403/, "ogni azione POST sui turni deve essere riservata all'admin");
assert.match(smartAppSource, /async function smartWorkingAction[\s\S]*?currentProfile\?\.role !== "admin"[\s\S]*?throw error;/, "il client deve bloccare localmente ogni modifica dei turni per lo staff");
assert.match(smartApiSource, /action === "sync_week"[\s\S]*?ensureCalendarRangeSynced\(actionWeekBounds/, "il server deve sincronizzare l'intervallo esatto della settimana");
...
assert.match(smartApiSource, /mondayOf\(assignment\.date\) < currentWeek[\s\S]*?targetDate < dateOnly\(new Date\(\)\)/, "il server deve consentire la settimana corrente senza accettare giorni trascorsi");
...
assert.match(smartApiSource, /if \(canManage\) await syncStaffProfilesToSmartWorking\(\);[\s\S]*?smart_work_employees\?select=\*&order=full_name\.asc/, "solo l'admin deve riconciliare i profili staff aprendo la pagina turni");
...
assert.match(smartApiSource, /can_move_smart: canManage/, "il calendario deve concedere lo spostamento soltanto all'admin");
