...
assert.match(htmlSource, /data-ped-full-drive/, "il selettore PED deve offrire l'accesso al Drive completo");
assert.match(appSource, /source === "all" \? "Drive completo" : data\.client\.name/, "la radice completa deve essere distinta dalla cartella cliente");
assert.match(appSource, /selectedFiles\.some\(\(file\) => file\.drive_source === "all"\)/, "un carosello deve ricordare i file scelti dal Drive completo anche dopo il ritorno al cliente");
assert.match(appSource, /drive_source: driveSource/, "il PED deve dichiarare al backend quando un file arriva dal Drive completo");
